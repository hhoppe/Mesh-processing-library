...
int main(int argc, const char** argv) {
    ...
    ARGSD(create_euler,         "yaw pitch roll : create frame from Euler angles (degrees)");
    ARGSC("",                   ":**");
    ARGSP(every,                "i : use only every ith element");
    ARGSP(object,               "obn : force object # to obn");
    args.f("-inverse", ginverse, ": normalize normals");
    ARGSP(pretransf,            "'frame' : pre-transform by frame");
    ARGSP(transf,               "'frame' : post-transform by frame");
    ARGSF(orthonormalize,       ": orthonormalize frame");
    ARGSP(induce_roll,          "factor : for flight over terrain");
    ARGSP(lowpass,              "factor : contribution of new frame [0..1]");
    ARGSF(add_mid_frames,       ": add interpolating frame between each pair");
    ARGSP(delay,                "fsec : introduce delay between frames");
    ARGSP(repeat,               "n : repeat every frame n times");
    ARGSF(frame,                ": introduce 'f 0 0 0' between frames");
    ARGSF(stat,                 ": output statistics");
    ARGSF(eof1,                 ": introduce 'q 0 0 0' after frame 1");
    ARGSF(toasciit,             ": force output to be ascii text");
    ARGSF(tobinary,             ": force output to be binary");
    args.parse();
    if (pretransf!="") {
        is_pretransf = true;
