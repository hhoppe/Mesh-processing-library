...
float get_filtered_pixel_value(const Vec2<int>& yxc) {
    ...
    float sum = valc; int count = 1;
    const int upper_range = 10;
    for_intL(range, 1, INT_MAX) {
        bool stop = false;
        float psum = 0.f; int pcount = 0;
        for (int sign : {-1, 1}) {
...
void do_tofloats(Args& args) {
    ...
    if (blocks) { begyx = V(bynum, bxnum)*blocks; endyx1 = begyx+blocks+1; }
    ...
    for (const auto& iyx : coords(numyx)) {
...
void do_tofmp(Args& args) {
    string filename = args.get_filename();
    if (image.zsize()>1)
        Warning("Only channel 0 (red) will be used to write fmp");
    WFile fi(filename);
    const int chw = 3;
...
void do_to(Args& args) {
    image.set_suffix(args.get_string());
}
...
void do_info() {
    ...
    Array<Stat> stat_pixels; for_int(z, image.zsize()) { stat_pixels.push(Stat(sform("Component%d", z))); }
    int na0 = 0, na255 = 0;
    ...
    if (0) {
        Stat stat("ch0"); stat.set_rms();
        for (const auto& yx : coords(image.dims())) { stat.enter(image[yx][0]); }
...
void do_cropmatte() {
    ...
    for (; l<image.xsize(); l++) {
        int x = l;
        bool ok = true; for_int(y, image.ysize()) { if (!equal(image[y][x], gcolor, nz)) { ok = false; break; } }
        if (!ok) break;
    }
    ...
    for (; t<image.ysize(); t++) {
        int y = t;
        bool ok = true; for_int(x, image.xsize()) { if (!equal(image[y][x], gcolor, nz)) { ok = false; break; } }
        if (!ok) break;
    }
...
void do_overlayimage(Args& args) {
    ...
    Image image2; image2.read_file(imagename);
    assertx(image.ok(yx0+image2.dims()-1));
    for (const auto& yx : coords(image2.dims())) { image[yx0+yx] = image2[yx]; }
}
...
void do_drawrectangle(Args& args) {
    ...
    int y1 = parse_size(args.get_string(), image.ysize(), true);
    const Vec2<int> p0(y0, x0), p1(y1, x1);
    for_coordsL(p0, p1, [&](const Vec2<int>& yx) {
        image[yx] = gcolor;
    });
}
...
void do_scalehalf2n1() {
    ...
    if (image.xsize()==1 && image.ysize()==1)
        Warning("Image already 1x1; no effect");
    const Vec2<int> newdims = (image.dims()-1)/2+1;
    assertx((newdims-1)*2+1==image.dims());
    Image timage(image);
    image.init(newdims);
    for (const auto& yx : coords(image.dims())) { image[yx] = timage[yx*2]; }
}
...
void do_blur(Args& args) {
    ...
    if (0) {
        {                    // normalize the 1D weights such that 2D tensor sums to 1 over discrete 2D window
            double sum = 0.; for_intL(dy, -r, r+1) for_intL(dx, -r, r+1) { sum += ar_gauss[dy+r]*ar_gauss[dx+r]; }
            for_int(i, 2*r+1) { ar_gauss[i] /= sqrt(float(sum)); if (0) SHOW(ar_gauss[i]); }
        }
...
void assemble_images(CMatrixView<Image> images) {
    for (const auto& yx : coords(images.dims())) { assertx(images[yx].zsize()==images[0][0].zsize()); }
    image.init(V(0, 0)); image.set_zsize(images[0][0].zsize()); // attributes copied outside this function
...
void do_assemble(Args& args) {
    ...
    for (const auto& yx : coords(images.dims())) {
        if (filenames[yx]=="") {
...
void do_fromtxt(Args& args) {
    int nx = args.get_int(), ny = args.get_int(); assertx(nx>0 && ny>0);
    int nch = args.get_int(); assertx(nch==1 || nch==3 || nch==4);
    string filename = args.get_filename();
    image.init(V(ny, nx)); image.set_zsize(nch);
    RFile fi(filename);
    Array<float> aval; aval.reserve(ny*nx*nch);
    for (const auto& yx : coords(image.dims())) {
        for_int(c, nch) {
            dummy_use(yx);
...
void do_invideo(Args& args) {
    ...
    for (int i = 0; ; i++) {
        ...
        ParseArgs parseargs{Array<string>{}}; parseargs.copy_parse(*g_parseargs);
        parseargs.parse();
...
void do_gridcrop(Args& args) {
    ...
    int sx = args.get_int(), sy = args.get_int(); assertx(sx>=0 && sx<=image.xsize() && sy>=0 && sy<=image.ysize());
    Matrix<Image> images(V(ny, nx));
    parallel_for_coords(images.dims(), [&](const Vec2<int>& yx) {
...
void do_disassemble(Args& args) {
    // Filterimage ~/data/image/lake.png -disassemble 128 128 rootname
    // Filterimage -assemble 2 2 rootname.{0.0,1.0,0.1,1.1}.png >lake.same.png
    int tilex = args.get_int(); assertx(tilex>0);
    int tiley = args.get_int(); assertx(tiley>0);
    ...
    string suffix = image.suffix(); assertx(suffix!="");
    parallel_for_coords(atiles, [&](const Vec2<int>& yx) {
        Image nimage(tiledims); nimage.attrib() = image.attrib();
        for (const auto& yxd : coords(tiledims)) {
...
void do_tile(Args& args) {
    const int nx = args.get_int(), ny = args.get_int(); assertx(nx>0 && ny>0);
    const Vec2<int> atiles(ny, nx);
    Image timage(image);
    image.init(timage.dims()*atiles);
    parallel_for_coords(atiles, [&](const Vec2<int>& yx) {
        for (const auto& yxd : coords(timage.dims())) {
...
template<int D> int64_t count_undef(CGridView<D,Pixel> grid) {
    return count_if(grid, [](const Pixel& pix) { assertx(pix[3]==0 || pix[3]==255); return pix[3]==0; });
}
...
template<int D> void quad_pullpush(GridView<D,Pixel> grid) {
    ...
    for (const auto& hu : coords(hgrid.dims())) {
        Vec3<int> accum(0, 0, 0); int num_def = 0;
        for (const auto& ud : coords(ntimes<D>(2))) {
            Vec<int,D> u = hu*2+ud;
...
template<typename Container> auto undo_alpha(Container&& c)
    -> decltype(map(c, std::declval<Vector4(*)(const Vector4&)>()))
{
...
template<typename Container> auto just_alpha(Container&& c)
    -> decltype(map(c, std::declval<float(*)(const Vector4&)>()))
{
...
void do_shadenor(Args& args) {
    Vector lightdir; for_int(c, 3) { lightdir[c] = args.get_float(); }
    assertx(image.zsize()==3);  // no alpha
    ...
    for (const auto& yx : coords(image.dims())) {
        ...
        Vector vnor;
        for_int(c, 3) {
            vnor[c] = float(pix[c])/255.f*2.f-1.f;
        }
...
void do_shadefancy(Args& args) {
    ...
    ld.push(Vector(-1.f, -.6f, +.3f)); lc.push(Vector(1.f, 1.f, 1.f)); li.push(.82f); ls.push(1.5f);
    ld.push(Vector(+.8f, +.4f, +.0f)); lc.push(Vector(.6f, .6f, .3f)); li.push(1.2f); ls.push(1.5f);
    ld.push(Vector(-.1f, +.9f, +.0f)); lc.push(Vector(.9f, .1f, .1f)); li.push(0.6f); ls.push(4.f);
    ld.push(Vector(-.1f, -.9f, +.0f)); lc.push(Vector(.9f, .1f, .1f)); li.push(0.6f); ls.push(4.f);
    ld.push(Vector(+.1f, +.0f, +.9f)); lc.push(Vector(.6f, .5f, .1f)); li.push(0.6f); ls.push(4.f);
    for_int(i, ld.num()) { ld[i].normalize(); ld[i] *= frame; }
    for (const auto& yx : coords(image.dims())) {
        Pixel& pix = image[yx];
        Vector vnor;
        for_int(c, 3) {
...
void do_cycle(Args& args) {
    int ncycle = args.get_int(); assertx(ncycle>0);
    // const int low = 40, high = 190, offset = low, range = high-low;
    const int low = 0, high = 255, offset = low, range = high-low;
    // use "-cycle 5" to see all the detail.
    int term = range/ncycle;
    for (const auto& yx : coords(image.dims())) {
        for_int(z, image.zsize()) {
...
void do_composite(Args& args) {
    ...
    Array<string> ops;
#define E(x) ops.push(#x); const int Op_##x = ops.num()-1
    E(blend);                   // weight
    E(special);
    E(shlomo);                  // weight
    E(min);
...
#undef E
    ...
    Image bimage;
    bimage.read_file(bfilename); assertx(same_size(bimage, image)); assertx(bimage.zsize()==image.zsize());
    for (const auto& yx : coords(image.dims())) {
        for_int(z, image.zsize()) {
...
void do_homogenize(Args& args) {
    ...
    int n = args.get_int(); assertx(n>=1 || n==-1);
    bool bilinear = false; if (n==-1) { bilinear = true; n = 2; }
    ...
    if (image.zsize()<4) {      // version without alpha-channel cropping
        ...
        // Derivation:
        //  We have  v = B*x  where rows of B are non-orthognal,  bb = B*B^T
        ...
        //             = x - B^T*R^-1*R^-T*B*x
        //             = x - B^T*(R^T*R)^-1*B*x
        //             = x - B^T*(B*B^T)^-1*B*x  (does not simplify because B is non-square, like (A^T*A)^-1*A^T*b.
...
inline Vector4 to_YIQ(const Vector4& pix) {
    return Vector4(dot(pix, Vector4(+0.299f, +0.587f, +0.114f, 0.f)),
                   dot(pix, Vector4(+0.596f, -0.274f, -0.322f, 0.f)),
...
void do_superresolution(Args& args) {
    ...
    float fac = args.get_float(); assertx(fac>1.f && fac<=32.f);
    ...
    parallel_for_coords(image.dims(), [&](const Vec2<int>& yx) {
        ...
        auto func_minmax = [&](float v) { if (v<lmin) lmin = v; if (v>lmax) lmax = v; };
        func_minmax(slum);
...
void do_istoroidal() {
    ...
    for_int(dir, 2) {
        ...
        for_int(i0, matrix.ysize()) {
            ...
            if (i1!=0) stat_int.enter(diff);
            else stat_bnd.enter(diff);
        }
    ...
    if (1) { std::cout << sform("%.2f\n", zconsec); std::cout.flush(); }
    nooutput = true;
}
...
void do_gdtoroidal() {
    ...
    if (0) {                    // solve for color values
        ...
        parallel_for_coords(image.dims(), [&](const Vec2<int>& yx) { // (highly inefficient implementation)
            Vector4 vrhs(0.f);
...
void do_gdfill() {
    ...
    if (0) {                    // solve for color values
        ...
        parallel_for_coords(image.dims(), [&](const Vec2<int>& yx) {
            ...
            Vector4 vrhs = -screening_weight*grid_orig[yx];
            if (!masked(yx)) {
                if (yx[0]>0   ) { auto p = yx+V(-1, 0); if (!masked(p)) vrhs += (grid_orig[p]-grid_orig[yx]); }
                if (yx[0]<ny-1) { auto p = yx+V(+1, 0); if (!masked(p)) vrhs += (grid_orig[p]-grid_orig[yx]); }
                if (yx[1]>0   ) { auto p = yx+V(0, -1); if (!masked(p)) vrhs += (grid_orig[p]-grid_orig[yx]); }
                if (yx[1]<nx-1) { auto p = yx+V(0, +1); if (!masked(p)) vrhs += (grid_orig[p]-grid_orig[yx]); }
            }
            multigrid.rhs()[yx] = vrhs;
        }, 50);
        ...
        multigrid.solve();
        auto grid_result = multigrid.result();
        for_coords(grid_result.dims(), [&](const Vec2<int>& yx) {
            HH_SSTAT(Sgrid_result0, grid_result[yx][0]);
        });
        parallel_for_coords(image.dims(), [&](const Vec2<int>& yx) {
...
void output_contour(int gn, float contour_value) {
    ...
    for (const auto& yx : coords(twice(gn-1))) {               // visit all contour cells
...
void do_niter(Args& args) {
    g_niter = args.get_int();
}
...
void do_poisson() {
    ...
    auto func_default_pos = [&](const Vec2<int>& yx) {
        ...
        return Point(((maxn-image.dim(1))/2.f+yx[1])/(maxn-1.f),
                     ((maxn-image.dim(0))/2.f+yx[0])/(maxn-1.f),
                     0.f);
    };
    float vmean = 0.f; for (const auto& yx : coords(image.dims())) { vmean += float(image[yx][0]); }
    vmean /= float(image.size());
    Matrix<Point> matp(image.dims());
    for (const auto& yx : coords(image.dims())) { matp[yx] = func_default_pos(yx); }
    ...
    for_int(iter, g_niter) {
        ...
        SparseLLS lls(2*(ny*(nx-1)+nx*(ny-1)) + conformal*2*(ny-2+conf_L)*(nx-2+conf_L) +nconstraints, ny*nx*2, 1);
        lls.set_verbose(0);
        ...
        //  the gradient is taken to be the mean of gradients at the two samples.
        {
            ...
            for_int(x, nx) for_int(y, ny-1) {
                ...
                if (!v) { Warning("Interpreting zero input differences as unconstrained differences"); w = smallw; }
        ...
        if (conformal) {
            if (conf_L) {
                ...
                if (iter>0) {
                    Matrix<float> matconf(ny-1, nx-1); double a = 0.;
                    HH_STAT(stat);
                    for (const auto& yx : coords(image.dims()-1)) {
    ...
    }
    {
        ...
        for (const auto& yx : coords(image.dims())) {
            ...
            if (1) {
                ...
                Point opos = func_default_pos(yx); mesh.update_string(v, "Opos", csform_vec(str, opos));
            }
        }
        for (const auto& yx : coords(image.dims()-1)) {
        ...
        hh_clean_up();
        mesh.write(std::cout); std::cout.flush(); nooutput = true;
    }
}
...
void do_procedure(Args& args) {
    ...
    if (0) {
        ...
        Image image1(image);
        Image image2; image2.read_file(args.get_filename());
        assertx(same_size(image1, image2));
        for (const auto& yx : coords(image.dims())) { image[yx] = yx[1]%2==0 ? image1[yx] : image2[yx]; }
    } else if (name=="stereo_purple") {
        const int n = 800; const int nobj = 8; const int r = 25; const int rg = 180;
        Image image1(V(n, n), Pixel::white());
        Image image2(V(n, n), Pixel::white());
        for (const auto& oyx : coords(twice(nobj))) {
            for (const auto& yx : coordsL(oyx*100+50-r, oyx*100+50+r)) {
        ...
        for (const auto& yxi : coords(twice(2))) {
            for (const auto& yx : coords(twice(n))) {
                const int beat = 8;
                ...
                if (is_on) {
                    if (0) void();
                    else if (yxi==V(0, 0)) pix = Pixel(255, 130, 130);
                    else if (yxi==V(0, 1)) pix = Pixel(40, 255, 40);
                    else if (yxi==V(1, 0)) pix = Pixel(150, 150, 255);
                    else if (yxi==V(1, 1)) pix = Pixel(255, 40, 255);
                    else assertnever("");
                }
                image[yxi*n+yx] = pix;
            }
        ...
        int gridn = args.get_int(); assertx(gridn>=1);
        ...
        Matrix<Vec2<int>> mvec(image.dims()); { // compute vectors to closest pixel in the mask
        ...
        Matrix<bool> visited(image.dims(), false); { // Grow mask to pixels with the same period.
            Queue<Vec2<int>> queue;
            for_coords(image.dims(), [&](const Vec2<int>& yx) {
                if (image[yx][0]>=cost_threshold) {
                    queue.enqueue(yx); visited[yx] = true;
                }
            });
            while (!queue.empty()) {
                ...
                for (const auto& yxd : {V(+1, 0), V(-1, 0), V(0, +1), V(0, -1)}) {
                    ...
                    if (!visited[yxn] && image_periods[yxn]==image_periods[yx]) { // compare RGB values of period
                        ...
                        queue.enqueue(yxn); visited[yxn] = true;
                    }
                }
            }
        }
        ...
        {            // Let the mask be a dilation unioned with the bfs result but limited to a maximum radius
            ...
            parallel_for_coords(image.dims(), [&](const Vec2<int>& yx) {
                ...
                bool in_mask = (radius2<=radius2_threshold ||
                                (visited[yx] && radius2<=radius2_max));
        ...
        Matrix<Image> grid_thumbnails; {
            ...
            for_coords(grid_thumbnails.dims(), [&](const Vec2<int>& yx) {
                grid_thumbnails[yx] = (ar_thumbnails.ok(i) ? std::move(ar_thumbnails[i++]) :
                                       Image(twice(size), gcolor));
        ...
        for_coords(image.dims(), [&](const Vec2<int>& yx) {
            ...
            assertx(pixel[0]==128); assertx(pixel[3]==255);
        ...
        Image image_mask;
        if (have_mask) { image_mask.read_file(mask_name); assertx(image_mask.dims()==image.dims()); }
        int est_num_input_frames = 0;
        for_coords(image.dims(), [&](const Vec2<int>& yx) {
            int staticframe = image[yx][0], start = image[yx][1], period = image[yx][2];
            int endframe = period<=1 ? start : start+period-1;
...
void do_diff(Args& args) {
    string filename = args.get_filename();
    Image image2; image2.read_file(filename);
...
void do_maxdiff(Args& args) {
    ...
    if (maxdiff>thresh) { SHOW(maxdiff, thresh); assertnever("maxdiff threshold exceeded"); }
}
...
void do_maxrmsdiff(Args& args) {
    ...
    if (stat.rms()>thresh) { SHOW(stat.rms(), thresh); assertnever("rms threshold exceeded"); }
}
...
void do_compare(Args& args) {
    ...
    {                        // normalize the 1D weights such that 2D tensor sums to 1 over discrete 2D window
        double sum = 0.; for_intL(dy, -r, r+1) for_intL(dx, -r, r+1) { sum += ar_gauss[dy+r]*ar_gauss[dx+r]; }
        for_int(i, 2*r+1) { ar_gauss[i] /= sqrt(sum); if (0) SHOW(ar_gauss[i]); }
    }
    if (0) {
        double sum = 0.;
        double var = 0.;
    ...
    for_int(z, image.zsize()) {
        ...
        for_int(y, image1.ysize())
            ...
            double mean1 = 0.;
            double mean2 = 0.;
            for (const auto& yxd : coordsL(twice(-r), twice(r+1))) {
...
inline Vec3<float> RGB_to_LAB(Vec3<float> v) {
    ...
    for_int(i, 3) {
        if (v[i] <= 0.04045f) v[i] /= 12.92f;
        else v[i] = pow((v[i]+0.055f)/1.055f, 2.4f);
    }
    ...
    v[2] = Z/1.08883f;
    for_int(i, 3) {
        if (v[i]>eps) v[i] = pow(v[i], 1/3.f);
        else v[i] = (k*v[i]+16.f)/116.f;
    }
    X = (116.f*v[1])-16.f;
    Y = 500.f*(v[0]-v[1]);
    Z = 200.f*(v[1]-v[2]);
...
inline Vec3<float> LAB_to_RGB(Vec3<float> v) {
    ...
    for_int(i, 3) {
        ...
        if (pf>0.008856f) v[i] = pf;
        else v[i] = (v[i]-16.f/116.f)/7.787f;
    }
    X = v[0]*0.95047f;
    Y = v[1]*1.0f;
    Z = v[2]*1.08883f;
...
void do_pyramid(Args& args) {
    ...
    HH_TIMER(_pyramid);
    string rootname = ffile; assertx(contains(rootname, '.'));
    rootname.erase(rootname.find('.')); // unlike get_path_root(), remove multiple extensions
    Image& imagec = image;
    Image imagef; imagef.read_file(ffile);
...
int main(int argc, const char** argv) {
    ...
    ARGSC("",                   ": (Image coordinates: (x=0, y=0) at left, top)");
    ARGSC("",                   ":An image is automatically read from stdin except with the following arguments:");
    ARGSD(nostdin,              ": do not attempt to read input image from stdin");
    ARGSD(create,               "width height : create white image");
    ARGSD(as_fit,               "nx ny : when assembling, scale each image uniformly to fit into this size");
    ARGSD(as_cropsides,         "l r t b : when assembling, crop each image");
    ARGSD(assemble,             "nx ny images_lr_tb_order : concatenate grid of images");
    ARGSD(fromtxt,              "nx ny nch file.txt : read values in range [0., 1.]");
    ARGSD(invideo,              "videofile : process each video frame, writing to a new video");
    ARGSC("",                   ":");
    ARGSD(to,                   "suffix : set output format (jpg, png, bmp, ppm, rgb, tif, wmp)");
    ARGSD(outfile,              "filename : output an intermediate image");
    ARGSF(nooutput,             ": do not output final image on stdout");
    ARGSD(info,                 ": print image statistics");
    ARGSD(stat,                 ": equivalent to -info -nooutput");
    ARGSD(sizes,                ": print 'width height'");
    ARGSD(tops,                 ": convert to encapsulated postscript");
    ARGSC("",                   ":");
    ARGSD(color,                "r g b a : set color for various operations (default 255's)");
    ...
    ARGSD(cropr,                "r : crop image");
    ARGSD(cropt,                "t : crop image");
    ARGSD(cropb,                "b : crop image");
    ARGSD(cropall,              "i : crop all sides");
    ARGSD(cropsquare,           "x y size : crop square centered at (x, y)");
    ARGSD(croprectangle,        "x y xsize ysize : crop rectangle centered at (x, y)");
    ARGSD(cropcoord,            "x0 y0 x1 y1 : crop box within bounds x0<=x<x1");
    ...
    ARGSD(scalehalf2n1,         ": subsample 4^n+1 -> 2^n+1 on each axis");
    ARGSC("",                   ":");
    ARGSD(flipvertical,         ": reverse rows");
    ARGSD(fliphorizontal,       ": reverse columns");
    ARGSD(rot180,               ": rotate by 180 degrees");
    ARGSD(rotate,               "ang : rotate ccw by ang degrees");
    ARGSD(gtransf,              "'frame' : geometrically transform image using (y, x, 0) 3D-coordinates)");
    ARGSD(tile,                 "nx ny : grid repeat");
    ARGSD(disassemble,          "tilex tiley rootname : break up into multiple image files of this size");
    ARGSD(gridcrop,             "nx ny sizex sizey : assemble grid of regions (with as_cropsides)");
    ARGSC("",                   ":");
    ARGSD(overlayimage,         "xl yt image : place image above current one");
    ARGSD(drawrectangle,        "x0 y0 x1 y1 : replace specified region by current color");
    ARGSD(replace,              "r g b a : replace all pixels matching specified color with this color");
    ARGSD(gamma,                "v : gammawarp image");
    ARGSD(tobw,                 ": convert to grayscale");
    ARGSD(tocolor,              ": convert to color");
    ARGSD(hue,                  ": just keep hue");
    ARGSD(noalpha,              ": remove an independent alpha channel");
    ARGSD(undoalpha,            ": remove alpha channel and adjust RGB values to undo premultiplication");
    ARGSD(readalpha,            "image_alpha : set alpha channel from Red channel of this image");
    ARGSD(setalpha,             "alpha : set alpha channel (0..255)");
    ARGSD(getalpha,             ": move alpha channel to RGB color channels");
    ARGSD(matchtoalpha,         ": set alpha channel based on whether pixel matches color");
    ARGSC("",                   ":");
    ARGSD(quadpullpush,         ": replace matching color based on quadtree");
    ARGSD(softpullpush,         ": more fancy, based on bicubic filter");
    ARGSD(voronoidilate,        ": replace all matching pixels with closest non-matching color");
    ARGSD(featureoffsets,       ": replace with vector to closest nonmatching");
    ARGSD(normalizenor,         ": for normal map, normalize |(r, g, b)| to 1");
    ARGSD(shadenor,             "dx dy dz : shade normal map given light_dir");
    ARGSD(shadefancy,           "'frame' : shade using multiple lights");
    ARGSD(cycle,                "n : cycle through colors");
    ARGSD(transf,               "'frame' : post-multiply RGB vector by a matrix (ranges [0..1])");
    ARGSD(genpattern,           "pat : create procedural pattern ([xydr]h+[slqc])");
    ARGSD(permutecolors,        ": randomize the unique colors");
    ARGSD(randomizeRGB,         ": randomize the unique colors, across channels too");
    ARGSD(noisegaussian,        "sd : introduce white Gaussian noise (in range 0..255)");
    ARGSD(blur,                 "r : apply Gaussian blurring with 1sdv = r pixels (e.g. 1.)");
    ARGSC("",                   ":");
    ARGSD(composite,            "op_name float back_image : blend");
    ARGSD(homogenize,           "periods : remove low frequencies (typically 4) (-1 is bilinear)");
    ARGSD(superresolution,      "fac : magnify smartly");
    ARGSD(istoroidal,           ": set exit_state>0 if image is nontoroidal");
    ARGSD(gdtoroidal,           ": use gradient-domain optimization to make toroidal");
    ARGSD(gradientsharpen,      "fac : apply gradient-based sharpening/attenuation (e.g. 1.5 or 0.7)");
    ARGSD(gdfill,               ": apply Poisson diffusion to alpha-masked regions");
    ARGSD(contour,              "gridres : output A3d contour at red-channel isovalue 127.5 (!gridres -> max(dims))");
    ARGSD(mcontours,            "gridres ncontours : output multiple contour curves");
    ARGSC("",                   ":");
    ARGSF(fixedbnd,             ": rectangular boundary for poisson");
    ARGSP(wconformal,           "w : weight on conformality constraint (e.g. 1.f)");
    ARGSP(gscale,               "fac : scale on input image gradient (typically used only if unconstrained regions)");
    ARGSD(niter,                "n : number of iterations of linear Poisson");
    ARGSD(poisson,              ": output poisson warping mesh");
    ARGSD(procedure,            "name... : performed named operation");
    ARGSD(diff,                 "image2 : compute difference 128+image-image2");
    ARGSD(maxdiff,              "fthresh image2 : crash if image value differs by more than threshold (range 0..255)");
    ARGSD(maxrmsdiff,           "fthresh image2 : crash if rms image diff exceeds threshold (range 0..255)");
    ARGSD(compare,              "image2 : compute difference statistics");
    ARGSD(resamplemesh,         "mesh.m : resample image colors onto warped mesh");
    ARGSC("",                   ":");
    ARGSP(use_lab,              "bool : use LAB rather than RGB");
    ARGSD(pyramid,              "imagef : structure transfer + CLB");
    ARGSD(structuretransfer,    "structureimg : inject detail");
    ARGSC("",                   ":");
    ARGSF(elevation,            ": input is elevation map (instead of color)");
    ARGSF(rg_elev,              ": elevation is 16bit (R, G channels)");
    ARGSP(scalezaxis,           "fac : for elevation, scale pixel values");
    ARGSP(offsetzaxis,          "off : for elevation, offset after scaling");
    ARGSF(removekinks,          ":  smooth out quantized pixel values");
    ARGSP(blocks,               "s : block size");
    ARGSP(step,                 "s : step size");
    ARGSP(bxnum,                "x : block number");
    ARGSP(bynum,                "y : block number");
    ARGSF(quads,                ": generate quads instead of triangles");
    ARGSF(strip_order,          ": different order for lru cache size 3");
    ARGSF(best_diagonal,        ": use shorter diagonal");
    ARGSF(toggle_order,         ": toggle triangle diagonal on each row");
    ARGSD(tomesh,               ": output mesh on stdout");
    ARGSD(tofloats,             "f.floats : output file of binary elevations");
    ARGSD(tofmp,                "f.fmp : output (X, Y, Z) binary floating-point");
