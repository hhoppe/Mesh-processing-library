...
class MapWedgeInfo {
 public:
    ...
    bool contains(int wid) const                { assertx(wid); return _map.contains(wid); }
    void set(int wid, WedgeInfo wi)             { assertx(wid); _map[wid] = std::move(wi); }
...
class LivingAncestor {
 public:
    ...
    void set_vertex(int vnew)                   { if (_is_active) _mapvidvid.enter(vnew, vnew); }
 private:
...
void LivingAncestor::activate() {
    assertx(!_is_active); _is_active = true;
...
WedgeInfo create_winfo(Corner c) {
    WedgeInfo wi;
    Vector& nor = wi.nor; fill(nor, k_undefined);
...
bool should_perform_vsplit(Vertex vs) {
    ...
    for_int(i, ar_pscreen.num()) {
        ...
        if (vcross>=0) facing_front = true;
        else facing_back = true;
...
bool parse_line(char* sline, bool& after_vsplit, bool carry_old) {
    ...
    if (!strcmp(sline, "# Beg REcol")) {
        if (!save0.skip_current) {
            if (carry_old)
                carry_old_corner_info(mesh.edge(save0.vs, save0.vt));
    ...
    if (!strncmp(sline, "REcol ", 6)) {
        ...
        save0.vs = vs; save0.vt = vt; save0.ii = ii;
        ...
        for (Corner cc : mesh.corners(v)) {
            if (mesh.face_id(mesh.corner_face(cc))==fi) { c = cc; break; }
        }
        if (!c) {
...
int get_unique_wid(Vertex v, bool use_old) {
    int num = 0, u_wid; dummy_init(u_wid);
    bool same_wid = true;
    for (Corner c : mesh.corners(v)) {
...
string create_attrib_string(const WedgeInfo& wi, bool old_flag) {
    ...
    if (nor[0]!=k_undefined)
        sret += sform(" %snormal=(%g %g %g)", sflag, nor[0], nor[1], nor[2]);
    if (col[0]!=k_undefined)
        sret += sform(" %srgb=(%g %g %g)", sflag, col[0], col[1], col[2]);
    if (uv[0]!=k_undefined)
        sret += sform(" %suv=(%g %g)", sflag, uv[0], uv[1]);
    return sret;
}
...
void write_init_vertex(Vertex v, bool write_morph) {
    ...
    assertx(u_nwid!=0);         // impossible
                                // if (write_morph) assertw(u_owid!=0); // apparently it's possible
...
bool next_morph(int nfaces) {
    ...
    }
    {
        ...
        if (num_will_be_constant)
            showdf("(%d corners not interpolated)\n", num_will_be_constant);
    }
    // Write morph.
...
void do_fbasemesh(Args& args) {
    ...
    for (Vertex v : mesh.vertices()) {
        for (Corner c : mesh.corners(v)) {
            ...
            c_cwedge_id(c) = wid;
            if (!gcwinfo.contains(wid))
                gcwinfo.set(wid, create_winfo(c));
        }
    }
...
void do_view(Args& args) {
    ...
    if (filename=="") { sel_refinement = false; return; }
    RFile fi(filename);
    for (;;) {
        if (fi().peek()<0) break;
        Frame f; int obn; float z; bool bin;
        if (!assertw(FrameIO::read(fi(), f, obn, z, bin))) break;
        if (obn) { Warning("Skipping non-object0 frame"); continue; }
...
unsigned encode_wst(int wid_l, int wid_r, int wid_lo, int wid_ro) {
    if (!wid_r) {
        ...
        return ((wid_l==wid_lo ? Vsplit::B_LSAME : 0u) |
                (wid_r==wid_ro ? Vsplit::B_RSAME : 0u) |
...
PMWedgeAttribD diff(const PMWedgeAttrib& a1, const PMWedgeAttrib& a2) {
    PMWedgeAttribD ad; diff(ad, a1, a2); return ad;
}
...
PMWedgeAttribD diff_zero(const PMWedgeAttrib& a1) {
    ...
    PMWedgeAttribD ad; diff(ad, a1, zero); return ad;
}
...
void process_vsplit() {
    ...
    if (save.vr) {
        ...
        if (nt && ns) {
            if (ut)
                vspl.ar_wad.push(diff_zero(get_wattrib(mesh.corner(save.vt, fr))));
            if (us)
                vspl.ar_wad.push(diff_zero(get_wattrib(mesh.corner(save.vs, fr))));
        } else {
    ...
    if (code&Vsplit::L_NEW)
        vspl.ar_wad.push(diff_zero(get_wattrib(mesh.corner(save.vl, fl))));
    if (code&Vsplit::R_NEW)
        vspl.ar_wad.push(diff_zero(get_wattrib(mesh.corner(save.vr, fr))));
    if (pm_has_wad2) {
    ...
    if (!strncmp(sline, "REcol ", 6)) {
        ...
        Edge esr = vr ? mesh.edge(vs, vr) : nullptr;
        {
            if (!vr) {
                ...
                vspl.vlr_offset1 = narrow_cast<short>(j+1); assertx(vspl.vlr_offset1==j+1);
            }
        }
        ...
        Vertex vt = mesh.split_vertex(vs, vl, vr, vti);
        // Save what was done
        save.vs = vs; save.vt = vt; save.ii = ii;
        save.vl = vl; save.vr = vr;
    } else if (sline[0]=='F' && !strncmp(sline, "Face ", 5)) {
...
void do_pm_encode() {
    ...
    for_int(i, pm_material_strings.num()) {
        bmesh._materials.set(i, pm_material_strings[i]);
    }
    ...
    Map<int,int> mwrenumber;
    {
        ...
        for (Vertex v : mesh.ordered_vertices()) {
            ...
            for (Corner c : mesh.corners(v)) {
                int wid = c_cwedge_id(c);
                bool is_new; int& i = mwrenumber.enter(wid, 0, is_new);
                if (!is_new) continue;
    ...
    // Parse vsplit records and encode.
    {
        ...
        for (;;) {
            ...
            if (!is) { assertx(!after_vsplit); break; }
            assertx(after_vsplit);
    ...
    if (append_old_pm!="") {
        ...
        int num_old_bmesh_faces;
        {
            ...
            for_int(i, materials.num()) {
                assertx(old_bmesh._materials.get(i)==materials.get(i));
            }
        ...
        for (;;) {
            ...
            vspl1.code = narrow_cast<ushort>((vspl1.code&~Vsplit::VSINDEX_MASK) |
                                             (new_vsindex<<Vsplit::VSINDEX_SHIFT));
            // note 'continue' above.
        }
...
int main(int argc, const char** argv) {
    ...
    ARGSD(fbasemesh,            "file.m : base mesh");
    ARGSD(fprogressive,         "file.rprog : progressive vsplit stream");
    ARGSC("",                   ":Next mostly made obsolete by FilterPM");
    ARGSP(maxnfaces,            "nfaces :  for arithseq & geomseq");
    ARGSD(arithseq,             "delta : geomorph every delta nfaces");
    ARGSD(geomseq,              "factor : geomorph every factor nfaces");
    ARGSD(to,                   "nfaces : geomorph up to nfaces");
    ARGSD(from,                 "nfaces : skip until at least nfaces");
    ARGSD(mesh,                 ": output current static mesh");
    ARGSD(at,                   "nfaces : -from nfaces -mesh");
    ARGSD(view,                 "s3d_filename : selective refinement");
    ARGSD(consider,             "nrecords : skip until at least nrecords");
    ARGSD(animateto,            "nfaces : animate vsplits up to nfaces");
    ARGSP(stringent,            "bool : use stringent test for sel. refin.");
    ARGSF(use_silhouette,       ": sel. refin. based on silhouette");
    ARGSF(use_area,             ": sel. refin. based on face area");
    ARGSP(splitcorners,         "b : split corners into vertices");
    ARGSC("",                   ":Construction of PM");
    ARGSP(append_old_pm,        "file.pm :  append old vsplit sequence");
    ARGSD(pm_encode,            ": output PM format");
    // HH_TIMER(Filterprog);
