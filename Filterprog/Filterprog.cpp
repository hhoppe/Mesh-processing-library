...
void process_vsplit() {
  ...
  // Encode vertex attributes.
  {
    ...
    if (!is_zero(vspl.vad_small.dpoint) && mag2(vspl.vad_small.dpoint) < 1e-14f)
...
void do_pm_encode() {
  ...
  std::cout.flush();
  if (!k_debug) hh_clean_up(), exit_immediately(0);  // Skip ~GMesh() and ~PMesh().
}
...
int main(int argc, const char** argv) {
  ...
  HH_ARGSP(stringent, "bool : use stringent test for selective refinement");
