...
int main(int argc, const char** argv) {
    ...
    ARGSP(title,                "name : set window title bar");
    ARGSP(hither,               "f : set hither plane distance");
    ARGSP(yonder,               "f : set yonder plane distance");
    ARGSF(eyeob,                ": make first object be eye object");
    ARGSP(zoom,                 "f : set focal length");
    ARGSF(iostat,               ": show I/O statistics");
    ARGSF(timestat,             ": show timing statistics");
    ARGSF(terse,                ": turn diagnositcs off");
    ARGSF(killeof,              ": exit upon reading EOF");
    ARGSF(spacekill,            ": exit upon space key press");
    ARGSP(statefilename,        "s3dname : name of .s3d file (frames)");
    ARGSP(caption,              "string : draw at bottom of window");
    ARGSD(key,                  "keystring : simulate key presses");
    ARGSD(frame,                "'frame' : change an object's frame");
    ARGSF(input,                ": read stdin even if have filenames");
    ARGSF(asynchronousinput,    ": update between EndFrames");
    ARGSF(subdivmode,           ": object 2 := subdiv. surface of 1");
    ARGSP(anglethresh,          "angle : sharp dihedral angle");
    args.other_args_ok();
