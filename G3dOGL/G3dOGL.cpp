...
class GXobject {
 public:
    ...
    void open(bool todraw)                      { assertx(!_opened); _opened = true; s_idraw = todraw; }
...
class GXobjects {
 public:
    ...
    void clear(int segn);
    void open(int segn);
    void add(const A3dElem& el)                 { assertx(_segn!=-1); _ob[_segn]->add(el); }
    void close()                                { assertx(_segn!=-1); _ob[_segn]->close(); _segn = -1; }
...
void update_mat_color2(const Color& col) {
    ...
    if (col.d!=matcol.d)
        fast_update_mat_diffuse(col.d);
    curcol = k_color_invalid;
}

inline void update_mat_color(const Color& col) {
    if (col.d!=matcol.d || col.s!=matcol.s || col.g!=matcol.g)
        update_mat_color2(col);
}

inline void maybe_update_mat_diffuse(const Pixel& cd) {
    if (cd!=matcol.d || force_color_update)
        fast_update_mat_diffuse(cd);
}
...
inline Vector interp_normal(const Vector& n1, const Vector& n2, float f1, float f2) {
    ...
    if (n1x==n2x && n1y==n2y && n1z==n2z) {
        ...
        if (denom) { nx /= denom; ny /= denom; nz /= denom; }
    }
...
void do_movie(Args& args) {
    ...
    movie_nframes = args.get_int(); assertx(movie_nframes>=2);
...
void DerivedHW::button_press(int butnum, bool pressed, const Vec2<int>& yx) {
    ...
    bool in_slider = (slidermode &&
                      !(pm_mode && yx_pointer_old[1]>=k_one_slider_left_thresh) &&
    ...
    if (pressed) {
        ...
        if (in_slider) {
            use_dl = was_using_dl; was_using_dl = false;
...
Vec2<int> find_max_texture(GLenum internal_format, const Vec2<int>& yx_aspect, int border,
                           int max_mipmap_level) {
    Vec2<int> max_yx = twice(0);
    assertx(!gl_report_errors());
    Vec2<int> yx; int level_offset; {
    ...
    for (int level = 0; ; ) {
        if (level) {
            ...
            glTexImage2D(GL_PROXY_TEXTURE_2D, 0, internal_format, yxt[1], yxt[0], border,
        ...
        }
        {
            ...
            glTexImage2D(GL_PROXY_TEXTURE_2D, level, internal_format, yxt[1], yxt[0], border,
                         GL_RGBA, GL_UNSIGNED_BYTE, nullptr);
        }
        if (glGetError()!=GL_NO_ERROR) break;
        int w;
    ...
    if (product(max_yx)) {
        ...
        glTexImage2D(GL_PROXY_TEXTURE_2D, 0, internal_format, max_yx[1], max_yx[0], border,
                     GL_RGBA, GL_UNSIGNED_BYTE, nullptr);
        int w; glGetTexLevelParameteriv(GL_PROXY_TEXTURE_2D, 0, GL_TEXTURE_WIDTH, &w);
        ...
        for_intL(mm, 1-detailed, 2) {
            for_intL(aspx, 1, 1+3*detailed+1) {
                ...
                showf("%-20s %d:%d (%4dx%4d) [%d,%d,%d,%d]%s%s\n",
                      glt_format.name.c_str(), aspyx[1], aspyx[0], max_yx[1], max_yx[0],
                      r, g, b, a, (comp ? " comp" : ""), (mm ? " mipmap" : ""));
            }
        }
        if (detailed) showf("\n");
    }
    ...
    {                           // matrices
        ...
        if (perspec) {
            glFrustum(-win_dims[1]*a*view_zoom*hither, win_dims[1]*a*view_zoom*hither,
                      -win_dims[0]*a*view_zoom*hither, win_dims[0]*a*view_zoom*hither,
                      hither, yonder);
        } else {
            glOrtho(-win_dims[1]*a*view_zoom, win_dims[1]*a*view_zoom,
                    -win_dims[0]*a*view_zoom, win_dims[0]*a*view_zoom,
                    hither, yonder);
        }
...
void mesh_init(GMesh& mesh) {
    ...
    Set<Vertex> vredo; Set<Face> fredo;
    Polygon poly;
    ...
    if (meshmodified) {
        ...
        for (Vertex v : vredo) {
            ...
            if (has_c_color) {
                if (mesh.flags(v).flag(vflag_color).set(false))
                    Warning("Found both vertex and corner rgb");
        ...
        for (Face f : fredo) {
            if (!mesh.is_triangle(f)) {
                mesh.polygon(f, poly);
                if (!poly.is_convex() && !num_concave++)
                    Warning("Have concave polygons in mesh");
            }
            A3dColor co;
    ...
    if (lsmooth) {
        ...
        for (Vertex v : vredo) {
            ...
            int num = 0;
            Vector gOnor, gNnor; dummy_init(gOnor, gNnor);
            for (Corner c : mesh.corners(v)) {
                ...
                if (mesh.parse_corner_key_vec(c, "Onormal", onor)) {
                    ...
                    clod.Onor = onor;
                    if (!num++) {
                        gOnor = clod.Onor; gNnor = clod.Nnor;
                    } else {
                        if (gOnor!=clod.Onor) uniquenors = false;
                        if (gNnor!=clod.Nnor) uniquenors = false;
                    }
                } else {
                    if (num) { Warning("missing Onormal"); uniquenors = false; }
                }
            }
    ...
    if (0 && strip_lines && !map_mfa.contains(&mesh)) {
        Array<Face> fa; for (Face f : mesh.ordered_faces()) { fa.push(f); }
        map_mfa.enter(&mesh, std::move(fa));
    }
    if (strip_lines) {
        bool is_new; Array<Face>& fa = map_mfa.enter(&mesh, Array<Face>(), is_new);
        if (is_new) { for (Face f : mesh.ordered_faces()) fa.push(f); }
    }
}
...
inline bool edge_continuous(const GMesh& mesh, Edge e) {
    {
        ...
        if (!same_corner_attrib(mesh, v1, mesh.ccw_corner(v1, e), mesh.clw_corner(v1, e)))
            return false;
    }
    {
        Vertex v2 = mesh.vertex2(e);
        if (!same_corner_attrib(mesh, v2, mesh.ccw_corner(v2, e), mesh.clw_corner(v2, e)))
            return false;
    }
    Face f1 = mesh.face1(e), f2 = mesh.face2(e); ASSERTX(f2);
    if (lsmooth && (mesh.flags(f1).flag(fflag_color)!=mesh.flags(f2).flag(fflag_color) ||
                    (mesh.flags(f1).flag(fflag_color) && f_color(f1)!=f_color(f2)))) return false;
    return true;
}
...
void draw_mesh(GMesh& mesh) {
        ...
        if (strict_mesh_order || (defining_dl && g_is_ati)) {
            ...
            for (Face f : mesh.faces()) {
                ...
                for (;;) {
                    ...
                    vi = va.index(vwaspiv); ASSERTX(vi>=0);
                    if (texture_active && !texture_lit) {
                    } else {
                        if (!lsmooth)
                            glNormal3fv(f_pnor(ff).data());
                        if (mesh.flags(ff).flag(fflag_color))
                            maybe_update_mat_diffuse(f_color(ff));
                    }
    ...
    if (ledges) {
        ...
        for (Edge e : mesh.edges()) {
            ...
            if (cullbackedges && lcullface && !uvtopos && !defining_dl) {
                ...
                for (Face f : mesh.faces(e)) {
                    if (dot(vtoe, f_pnor(f))<0) { cull = false; break; }
                }
                if (cull) continue;
            }
            int ithick;
        ...
        if (sphereradius) {
            ...
            bool is_new; float& mesh_radius = mesh_radii.enter(&mesh, 0.f, is_new);
            if (is_new) {
                Bbox bbox; bbox.clear(); for (Vertex v : mesh.vertices()) { bbox.union_with(mesh.point(v)); }
    ...
    if (strip_lines && !lquickmode) {
        ...
        Array<Vertex> va; Point p; Point pp; Face fp = nullptr;
        ...
        for (Face f : fa) {
            ...
            if (afound && cullbackedges && lcullface) {
                if (dot(p-feyetomodel.p(), f_pnor(f))>0) afound = 0;
                else if (dot(pp-feyetomodel.p(), f_pnor(fp))>0) afound = 0;
            }
            if (afound) {
                if (!stripstarted) {
                    stripstarted = true;
...
void draw_all() {
    ...
    for (int i = g_xobs.min_segn(); i<=g_xobs.max_segn(); i++) {
        ...
        if (use_dl) {
            bool is_new; int& dlnum = mdlnum.enter(i, 0, is_new);
        ...
        if (g_xobs[i].pmesh)
            g_xobs[i].pmesh->gflags().flag(g3d::mflag_ok) = true; // if sc_mode, psc_mode, etc.
        //
        if (defining_dl) {
            defining_dl = false;
...
void process_print() {
    ...
    glPushAttrib(GL_PIXEL_MODE_BIT); { // save GL_READ_BUFFER
        ...
        if (nxpix<44) {
            ...
            for_int(y, nypix) {
                glReadPixels(0, y, nxpix, 1, GL_RGBA, GL_UNSIGNED_BYTE, image[y].data());
            }
        } else {
    ...
    if (movie_desire_video) {
        ...
        if (movie_nframes)
            name = sform("%s.%03d.bmp", movie_rootname.c_str(), movie_frame);
...
void all_reset() {
    ...
    if (0) {
        ambient = .30f;
        lightsource = .65f;
        anisotropy = INT_MAX; set_anisotropy();
    }
}
...
void GXobject::morph(float finterp) { // finterp==1.f is new,   finterp==0.f is old
    ...
    if (!lsmooth || (ledges && cullbackedges && lcullface)) {
        if (0) {
            ...
            ogl_normalize = true;
            for (Face f : mesh.faces()) {
                Corner c1; dummy_init(c1);
                for (Corner c : mesh.corners(f)) { c1 = c; if (1) break; }
...
bool HB::init(Array<string>& aargs,
    ...
    ARGSP(edgecolor,                            "#RRGGBB : set 'De' color");
    ARGSP(sharpedgecolor,                       "#RRGGBB : ''");
    ARGSP(bndedgecolor,                         "#RRGGBB : ''");
    ARGSP(cullbackedges,                        "b : cull backfacing 'De' edges");
    ARGSF(outside_frustum,                      ": view larger than frustum");
    ARGSP(frustum_frac,                         "f : set fraction larger if outside");
    ...
    ARGSP(dbuffer,                              "val : set double buffering");
    ARGSF(picture,                              ": output picture and end");
    ARGSD(movie,                                "nframes rootname : create movie as set of rootname.%03d.bmp files");
    ARGSD(video,                                "nframes name.mp4 : create movie (360 frames for one 'J' rotation)");
    ...
    ARGSP(sphereradius,                         "f : tagged vertices radius");
    ARGSP(spherecolor,                          "r g b : tagged vertices color");
    ARGSP(meshcold,                             "r g b : mesh color diffuse");
    ...
    ARGSF(hpix,                                 ": do standard work for profiling");
    ARGSF(worldlight,                           ": bind light to world coordinates");
    ARGSP(fdisplacepolygon,                     "f : change OpenGL setting");
#if defined(DEF_SR)
    ARGSP(sr_screen_thresh,                     "t : fraction window radius");
    ARGSP(sr_regulatenf,                        "nfaces : regulate # faces");
    ARGSP(sr_fracvtrav,                         "f : fraction active verts to traverse");
    ARGSP(sr_gain,                              "val : set regulator gain");
    ARGSP(sr_gtime,                             "val : # frames for geomorph refinement");
#endif
    ARGSP(noinfo,                               "bool : do not show info or sliders");
...
void HB::draw_space() {
    ...
    if (outside_frustum) {
        ...
        if (thicknormal>1) {
            ...
            x1 -= yxd[1]; x2 += yxd[1]; y1 -= yxd[0]; y2 += yxd[0];
            HB::draw_segment(V(y1, x1), V(y1, x2));
            HB::draw_segment(V(y2, x1), V(y2, x2));
            HB::draw_segment(V(y1, x1), V(y2, x1));
            HB::draw_segment(V(y1, x2), V(y2, x2));
...
string HB::show_info() {
    ...
    return sform("[GL %c%c%c%c%c%c%c%c%c%c%c%c%c]",
                 g_xobs.cullface[obn] ? 'b' : ' ',
                 g_xobs.reverse_cull[obn] ? 'r' : ' ',
                 g_xobs.shading[obn] ? 's' : ' ',
                 g_xobs.smooth[obn] ? 'm' : ' ',
                 g_xobs.edges[obn] ? 'e' : ' ',
                 mdepthcue ? 'u' : ' ',
                 antialiasing ? 'a' : ' ',
...
void sr_regulator() {
    ...
    if (!product(win_dims)) {   // window is iconified on _WIN32
        ...
        if (abs(cur_nfaces-goal_nfaces)>10)
            sr_screen_thresh *= pow(cur_nfaces/goal_nfaces, sr_gain);
...
void sr_wrap_draw(bool show) {
    ...
    if (show && g3d::info) {
        ...
        string s3 = (!morph ? "" :
                     !bigfont() ?
                     sform(" morph%02d vgr=%04.1f%% vgc=%04.1f%%",
                           sr_gtime,
    ...
    switch (ch) {
     ...
     case 'r': {
         ...
         if (0) sr_tview = Frame(Vector(0.43588989f, 0.f, -0.9f),
                                 Vector(0.f, 1.f, 0.f),
                                 Vector(0.9f, 0.f, 0.43588989f),
                                 Point(-0.65f, 0.f, 2.48f));
         const float vv = 0.25f;
         if (1) sr_tview = Frame(Vector(vv, 0.f, -sqrt(1.f-square(vv))),
                                 Vector(0.f, 1.f, 0.f),
                                 Vector(sqrt(1.f-square(vv)), 0.f, vv),
                                 Point(-0.25f, 0.f, 2.8f));
         if (getenv_bool("G3D_RADAR_LOWER_SKEWED_RIGHT"))
             sr_tview = Frame(Vector(0.249389f, 0.0699849f, -0.965879f),
                              Vector(-0.0175022f, 0.997555f, 0.0677609f),
                              Vector(0.968246f, 6.13302e-06f, 0.25f),
                              Point(-0.0890237f, 0.021173f, 1.53995f));
...
void vertSmoothNormal(Simplex vs, Simplex corner_fct, Vector& avg_norm) {
    ...
    int i_vs = -1;
    for_int(i, 3) { if (vs==verts[i]) i_vs = i; }
    assertx(i_vs>=0);
    ...
    if (!done) {
        ...
        while (e->isManifold()) {
            ...
            // find va among verts
            int i_va = -1;
            for_int(i, 3) { if (va == verts[i]) i_va = i; }
            assertx(i_va>=0);
...
void read_sc(const string& filename) {
    ...
    ForSCSimplex(Kmesh, 2, s2) {
        Vec3<Simplex> v; s2->vertices(v.data());
        fct_pnor[s2->getId()] = ok_normalized(cross(v[0]->getPosition(), v[1]->getPosition(), v[2]->getPosition()));
    } EndFor;
    ForSCSimplex(Kmesh, 2, s2) {
        Vec3<Simplex> verts; s2->vertices(verts.data());
        for_int(i, 3) {
            vertSmoothNormal(verts[i], s2, corner_pnor[3*s2->getId()+i]);
        }
    } EndFor;
    ...
    if (new_lod_num < psc_lod_num) {
        ...
        for (int i = psc_lod_num-1; i>=new_lod_num; --i) {
            ...
            if (psc_unify_normal_list[i]) {
                ...
                int cnt = 0;
                ForSCVertexFace(vs, f) { dummy_use(f); cnt++; } EndFor;
                ...
                ForSCVertexFace(vs, s2) {
                    ...
                    nr.fct_nor = ok_normalized(cross(verts[0]->getPosition(),
                                                     verts[1]->getPosition(),
                                                     verts[2]->getPosition()));
                    fct_pnor.access(nr.fid);
                    fct_pnor[nr.fid] = nr.fct_nor;
                ...
                int ii = 0;
                ForSCVertexFace(vs, s2) {
                    Vec3<Simplex> verts2; s2->vertices(verts2.data());
    ...
    if (new_lod_num > psc_lod_num) {
        ...
        for (int i = psc_lod_num; i < new_lod_num; i++) {
            ...
            if (psc_split_normal_list[i]) {
                ...
                int cnt = 0;
                ForSCVertexFace(vs, f) { dummy_use(f); cnt++; } EndFor;
                ForSCVertexFace(vt, f) { dummy_use(f); cnt++; } EndFor;
                Array<NormalRecord>& anr = *new Array<NormalRecord>;
...
void draw_sc() {
    ...
    if (!ledges || lshading) {
        ...
        int i = 0;
        ForSCSimplex(Kmesh, 2, s2) {
            if ((++i&0x7F)==0 && i) { glEnd(); glBegin(GL_TRIANGLES); }
            ...
            glNormal3fv(n0.data()); glVertex3fv(p0.data());
            glNormal3fv(n1.data()); glVertex3fv(p1.data());
            glNormal3fv(n2.data()); glVertex3fv(p2.data());
        } EndFor;
        glEnd();
        // draw all 1-simplices with no parents
        psc_orphan_nedges = 0;
        glPushAttrib(GL_TRANSFORM_BIT); { // save GL_NORMALIZE
            glEnable(GL_NORMALIZE);
            for (Simplex s1 : psc_principal_edges) {
                ...
                float area = s1->getArea(); assertx(area>=0.f);
        ...
        for (Simplex s0 : psc_principal_verts) {
            ...
            float area = s0->getArea(); assertx(area>=0.f);
...
void read_ply(const string& filename) {
    // HH_TIMER(_read_ply);
    RFile fi(filename);
    { string sline; assertx(my_getline(fi(), sline)); assertx(sline=="ply"); }
    bool binary = false; bool bigendian = false;
    ...
    assertx(vnpos==3); assertx(vnnor==0 || vnnor==3); assertx(vnrgb==0 || vnrgb==3 || vnrgb==4);
    ...
    if (binary) {
        for_int(i, ply_vpos.num()) {
            ...
            for_int(c, vnpos) {
                if (bigendian) from_std(&ply_vpos[i][c]); else from_dos(&ply_vpos[i][c]);
            }
            if (vnnor) assertx(read_binary_raw(fi(), ply_vnor[i].view()));
            for_int(c, vnnor) {
                if (bigendian) from_std(&ply_vnor[i][c]); else from_dos(&ply_vnor[i][c]);
            }
            for_int(c, vnrgb) {
                assertx(read_binary_raw(fi(), ArView(ply_vrgb[i][c])));
            }
            for_int(c, vnotherb) {
                uchar dummy; assertx(read_binary_raw(fi(), ArView(dummy)));
            }
        }
        for_int(i, ply_findices.num()) {
            int ni;
            if (len_nfv==1) {
                uchar vi = 0; assertx(read_binary_raw(fi(), ArView(vi))); ni = vi;
            } else if (len_nfv==4) {
                int32_t vi = 0; assertx(read_binary_raw(fi(), ArView(vi))); ni = vi;
            } else { assertnever(""); }
            ply_findices[i].init(ni);
            for_int(j, ni) {
                assertx(read_binary_raw(fi(), ArView(ply_findices[i][j])));
                if (bigendian) from_std(&ply_findices[i][j]); else from_dos(&ply_findices[i][j]);
        ...
        for_int(i, ply_vpos.num()) {
            for_int(c, vnpos) {
                assertx(fi() >> ply_vpos[i][c]);
            }
            for_int(c, vnnor) {
                assertx(fi() >> ply_vnor[i][c]);
            }
            for_int(c, vnrgb) {
                int v; assertx(fi() >> v); ply_vrgb[i][c] = narrow_cast<uchar>(v);
            }
            for_int(c, vnother) {
                float dummy; assertx(fi() >> dummy);
            }
        }
        // SHOW(ply_vpos.last());
        for_int(i, ply_findices.num()) {
            int ni = 0; assertx(fi() >> ni);
            ply_findices[i].init(ni);
            for_int(j, ni) {
    ...
    Bbox bbox; bbox.clear(); for_int(i, ply_vpos.num()) { bbox.union_with(ply_vpos[i]); }
...
void draw_ply() {
    ...
    if (!ledges || lshading) {
        ...
        for_int(i, ply_findices.num()) {
            ...
            if (!lsmooth) {
                Vector fnormal = ok_normalized(cross(ply_vpos[ply_findices[i][0]],
                                                     ply_vpos[ply_findices[i][1]],
                                                     ply_vpos[ply_findices[i][2]]));
