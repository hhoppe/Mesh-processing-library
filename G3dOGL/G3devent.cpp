...
void save_state() {
    ...
    try {
        ...
        Frame ft; FrameIO::create_not_a_frame(ft);
...
void read_state() {
    ...
    try {
        ...
        string sline;
        while (fi().peek()>=0) {
            if (fi().peek()=='#') { assertx(my_getline(fi(), sline)); continue; }
...
void enter_aim() {
    ...
    Vector vf = g_obs[obview].t().v(0); assertx(vf.normalize());
    Vector vt = vaim; assertx(vt.normalize());
    Quaternion rot(vf, vt);     // get twice the angle!
    rot = pow(rot, .5f);        // divide angle in half
    Frame& frame = g_obs[obview].tm();
    Point pt = frame.p(); frame.p() = Point(0.f, 0.f, 0.f);
    frame *= to_Frame(rot);
    frame.p() = pt;
}
...
bool select_vertex(const Vec2<float>& yx, int& ret_obn, GMesh*& ret_mesh, Vertex& ret_v) {
    ...
    for (int obn = g_obs.first; obn<=g_obs.last; obn++) {
        ...
        for (Vertex v : mesh.vertices()) {
            ...
            float xp, yp, zp; HB::world_to_vdc(p, xp, yp, zp);
    ...
    for (int obn = g_obs.first; obn<=g_obs.last; obn++) {
        ...
        for (Edge e : mesh.edges()) {
            ...
            for (Face f : mesh.faces(e)) {
                ...
                float xs, ys, zs; HB::world_to_vdc(pn*t, xs, ys, zs);
    ...
    for (int obn = g_obs.first; obn<=g_obs.last; obn++) {
        ...
        for (Face f : mesh.faces()) {
            ...
            for_int(vi, va.num()-2) { // for non-triangles, create fan.
                ...
                Vec3<Point> psa; Vec3<float> psz, areas;
                bool all_in = true;
                for_int(i, 3) {
                    psa[i] = Point(0.f, 0.f, 0.f);
                    if (!HB::world_to_vdc(pa[i]*t, psa[i][0], psa[i][1], psz[i])) { all_in = false; break; }
                }
                if (!all_in) continue;
                Point ps(concat(yx.rev(), V(0.f)));
                for_int(i, 3) {
                    areas[i] = determ2d(psa[mod3(i+1)], psa[i], ps)*.5f;
                }
...
void swap_edge() {
    ...
    int obn; GMesh* mesh; Edge e; Point pint;
    if (!select_edge(yx, obn, mesh, e, pint)) { HB::beep(); return; }
    if (!mesh->legal_edge_swap(e)) { HB::beep(); return; }
...
void collapse_edge(bool tovertex) {
    ...
    int obn; GMesh* mesh; Edge e; Point pint;
    if (!select_edge(yx, obn, mesh, e, pint)) { HB::beep(); return; }
    if (!mesh->legal_edge_collapse(e)) { HB::beep(); return; }
    Vertex v1 = mesh->vertex1(e), v2 = mesh->vertex2(e);
    Point p1 = mesh->point(v1), p2 = mesh->point(v2);
...
bool try_toggle_edge() {
    Vec2<float> yx; if (!HB::get_pointer(yx)) return false;
    int obn; GMesh* pmesh; Edge e; Point pint;
...
void toggle_tag() {
    ...
    if (!(tagvertices && try_toggle_vertex()) && !try_toggle_edge()) { HB::beep(); return; }
    if (subdivmode) ClearSubMesh();
    HB::redraw_later();
}
...
void untag_cut() {
    Vec2<float> yx; if (!HB::get_pointer(yx)) return;
    int obn; GMesh* mesh; Edge e; Point pint;
    ...
    HB::redraw_later();
    {
        ...
        {
            Edge ee = er; Vertex v = mesh->vertex2(ee);
            for (;;) {
                are = get_sharp_edges(*mesh, v); if (are.num()!=1) break;
                ee = are[0];
                untag(*mesh, ee); v = mesh->opp_vertex(v, ee);
...
void destroy_face() {
    Vec2<float> yx; if (!HB::get_pointer(yx)) { HB::beep(); return; }
    int obn; GMesh* mesh; Face f;
    if (!select_face(yx, obn, mesh, f)) { HB::beep(); return; }
    for (Vertex v : mesh->vertices(f)) {
        mesh->flags(v).flag(vflag_ok) = false;
    }
    mesh->destroy_face(f);
...
void crop_mesh_to_view() {
    ...
    if (!assertw(mesh.num_faces())) { HB::beep(); return; }
    {
        ...
        for (Face f : mesh.faces()) {
            ...
            for (Vertex v : mesh.vertices(f)) {
                float xo, yo, zo;
                if (HB::world_to_vdc(mesh.point(v)*t, xo, yo, zo) &&
                    xo>=0.f && xo<=1.f && yo>=0.f && yo<=1.f) { all_out = false; break; }
            }
...
void select_frel() {
    ...
    for_int(i, 3) {
        for_int(j, 3) {
            ...
            if (v<0) { v = -v; sign = -1; }
            if (v>f1) {
                f2 = f1; oax2 = oax1; vax2 = vax1; sign2 = sign1;
                f1 = v; oax1 = i; vax1 = j; sign1 = sign;
            } else if (v>f2) {
                f2 = v; oax2 = i; vax2 = j; sign2 = sign;
            }
        }
    }
...
bool KeyPressed(const string& ps) {
    ...
    static char lastch;
    char thisch = ch;
    if (keylock && !contains("-=_+xd\tc~ Di? BQ[] avV be ol ^\x03", ch))
        return false;
    if (lastch=='D') {
        if (ch=='D') return true;
    ...
    if (editmode) {
        switch (ch) {
         ...
         bcase '@':
            ...
            if (lastch!=thisch) {
                if (cob<1 || cob>g_aargs1.num()) { SHOW("cannot write"); break; }
...
void ButtonPressed(int butnum, bool pressed, bool shift, const Vec2<float>& yx) {
    if (pressed) {
        ...
        if (editmode && butnum!=1) {
            if (!select_vertex(yx, selected.obn, selected.mesh, selected.v))
                HB::beep();
        }
        selected.frel = Frame::identity();
        if (!object_mode && cob!=obview) select_frel();
        if (ratemode==ERatemode::step) HB::redraw_now();
        else HB::redraw_later();
    } else {
