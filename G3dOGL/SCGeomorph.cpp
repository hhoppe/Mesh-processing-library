...
void SCGeomorph::vertSmoothNormal(Simplex vs, Simplex corner_fct, Vector& avg_norm, bool skip_degenerate) {
    ...
    while (e->isManifold()) {
        ...
        if (skip_degenerate && degenerate(verts)) {
            ...
            if (s_norgroup[fct->getVAttribute()] != ngroup)
                break;
...
void SCGeomorph::read(std::istream& is) {
    ...
    ForSCSimplex(K, 0, v) {
        ...
        if (soa) {
            float area; assertx(sscanf(soa, "%g", &area)==1);
            aold.enter(v, area);
        ...
        if (v->isPrincipal()) {
            if (0) {
                ...
                float area; assertx(sscanf(sna, "%g", &area)==1);
                anew.enter(v, area);
            } else {
    ...
    ForSCSimplex(K, 2, f) {
        Simplex v[3];
        f->vertices(v);
        fct_pnor[f->getId()] = ok_normalized(cross(v[0]->getPosition(),
                                                   v[1]->getPosition(),
                                                   v[2]->getPosition()));
    } EndFor;
    ...
    ForSCSimplex(K, 2, f) {
        ...
        f->vertices(verts);
        for_int(i, 3) {
            vertSmoothNormal(verts[i], f, nnew[3*f->getId()+i]);
        }
    } EndFor;
}
