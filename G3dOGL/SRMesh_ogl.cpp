...
template<bool use_texture> void SRMesh::ogl_render_faces_strips_aux() {
    ...
    for (EListNode* n = ndelim->next(); ; ) {
        ...
        const SRAVertex* v1n; const SRAVertex* v2n;
        SRAFace* fn;            // next neighboring face
#if defined(SR_SW_CULLING)
        int sw_culling_3bits = ((f->vertices[0]->visible<<0) |
                                (f->vertices[1]->visible<<1) |
                                (f->vertices[2]->visible<<2));
...
int SRMesh::ogl_render_striplines() {
    ...
    _cur_frame_mask = _cur_frame_mask^k_Face_visited_mask;
    for (SRAFace* ff : EList_outer_range(_active_faces, SRAFace, activef)) {
        ...
        f->matid = matidv;
        const SRAVertex* v1n; const SRAVertex* v2n; const SRAVertex* vn;
        if (0) {
        } else if (fn = f->fnei[0], HH_ASSUME(fn), fn->matid==matid) {
            fn->matid = matidv;
            v1n = f->vertices[0]; v2n = f->vertices[1]; vn = f->vertices[2];
        } else if (fn = f->fnei[1], HH_ASSUME(fn), fn->matid==matid) {
            fn->matid = matidv;
            v1n = f->vertices[1]; v2n = f->vertices[2]; vn = f->vertices[0];
        } else if (fn = f->fnei[2], HH_ASSUME(fn), fn->matid==matid) {
            fn->matid = matidv;
            v1n = f->vertices[2]; v2n = f->vertices[0]; vn = f->vertices[1];
        } else {
            fn = nullptr;
            v1n = f->vertices[0]; v2n = f->vertices[1]; vn = f->vertices[2];
        }
        ...
        for (;;) {
            ...
            if (vrand==v2n) {
                ...
                if (v1oldest) {
                    fn = fn->fnei[2];
                    if (fn->matid!=matid) { fn = nullptr; } else {
                        fn->matid = matidv; v1n = vn; v1oldest = false;
                    }
                } else {
                    fn = fn->fnei[0];
                    if (fn->matid!=matid) { fn = nullptr; } else {
                        fn->matid = matidv; v2n = vn; v1oldest = true;
                    }
                }
            } else if (vrand==v1n) {
                vn = fn->vertices[2];
...
static inline void sr_draw_segment(const SRAVertex* v0, const SRAVertex* v1,
                                   bool hasfn, const Point& p0, const Point& p1) {
     ...
     GOTO_STRIP_RESTART_FROM_SCRATCH:
        ...
        for (;;) {              // form one strip
            for (;;) {          // form ring(s) while having no fnext
                ...
                if (v0->cached_time<cache_time)
                    v0->cached_time = (cache_time++)+k_cache_size;
                if (v1->cached_time<cache_time)
                    v1->cached_time = (cache_time++)+k_cache_size;
                if (v2->cached_time<cache_time)
                    v2->cached_time = (cache_time++)+k_cache_size;
                p = interp(get_point(v0), get_point(v1), get_point(v2));
                glVertex3fv(p.data());
                SRAFace* fint = f->fnei[mod3(j+1)];
                SRAFace* fext = f->fnei[j];
                if (fint->matid==matid) {
                    f = fint; j = get_vf_j0(v0, f);
                    if (fext->matid==matid) {
                        fnext = fext; jnext = get_vf_j0(v2, fext);
                        cache_time_next = cache_time;
                        break;
                    }
                } else if (fext->matid==matid) {
                    f = fext; j = get_vf_j0(v2, f);
                } else {
                    glEnd();
...
void SRMesh::ogl_render_faces_tvc(bool unlit_texture) {
    ...
    for (EListNode* n = ndelim->next(); ; ) {
     GOTO_STRIP_RESTART_FROM_SCRATCH:
        ...
        SRAFace* fnext; int jnext; int cache_time_next;
        ...
        for (;;) {              // form one strip
            for (;;) {          // form ring(s) while having no fnext
                ...
                glNormal3fv(get_normal(v0).data()); glVertex3fv(get_point(v0).data());
                glNormal3fv(get_normal(v1).data()); glVertex3fv(get_point(v1).data());
                glNormal3fv(get_normal(v2).data()); glVertex3fv(get_point(v2).data());
                if (v2->cached_time<cache_time)
                    v2->cached_time = (cache_time++)+k_cache_size;
                SRAFace* fint = f->fnei[mod3(j+1)];
                SRAFace* fext = f->fnei[j];
                if (fint->matid==matid) {
                    f = fint; j = get_vf_j0(v0, f);
                    v1 = v2; v2 = f->vertices[mod3(j+2)];
                    if (fext->matid==matid) {
                        fnext = fext; jnext = get_vf_j0(v1, fext);
                        cache_time_next = cache_time;
                        break;
                    }
                } else if (fext->matid==matid) {
                    f = fext; j = get_vf_j0(v2, f);
                    v0 = v2; v2 = f->vertices[mod3(j+2)];
                } else {
                    goto GOTO_STRIP_RESTART_FROM_SCRATCH; // "continue" on outermost loop
                }
            }
            for (;;) {          // form ring(s) while having fnext
                ...
                // Decide to add next ring (e.g. 2 faces) or restart.
                int nf = 1; {
                    SRAFace* ff = f; int jj1 = mod3(j+1);
                    for (;;) {
                        ff = ff->fnei[jj1];
                        if (ff->matid!=matid) break;
                        nf++; jj1 = get_vf_j1(v0, ff);
                    }
                }
                if (cache_time-cache_time_next+(nf-1)>desiredloc) break;
            }
