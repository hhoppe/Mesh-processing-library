...
void SplitRecord::write(std::ostream& os) const {
    os << _vsid << " " << _vtid << "\n";
    for_int(i, _outcome.num()-1) {
        os << _outcome[i];
    }
    os << "\n";
    os << _pos_bit << " " << _deltap[0] << " "
       << _deltap[1] << " " << _deltap[2] << "\n";
...
void SplitRecord::applySplit(SimplicialComplex& K) {
    ...
    for_int(i, _material.num()) {
        Simplex s = K.getSimplex(_material[i].dim,
                                 _material[i].id);
    ...
    for_int(i, _area.num()) {
        Simplex s = K.getSimplex(_area[i].dim,
                                 _area[i].id);
        assertx(s && s->isPrincipal());
...
void SplitRecord::applyUnify(SimplicialComplex& K) const {
    ...
    if (_pos_bit==0)
        new_pos = interp(vs->getPosition(), vt->getPosition());
