...
float dihedral_penalty(const Dihedral& dih, const NewMeshNei& nn, const Point& newp) {
  ...
  if (!bad) {
    ...
    bad = ranges::any_of(nn.ar_corners, [&](const Vec3<Corner>& corners) {
      ...
      return is_sliver(p0, p1, newp) && !is_sliver(p0, p1, mesh.point(mesh.corner_vertex(corners[2])));
    });
  }
      ...
      for (Corner crep : mesh.corners(v)) {
        ...
        for_int(dir, 2) {  // Two directions (CCW, CLW).
          ...
          for (;;) {
            ...
            if (nwidfound && sdebug && !diff) assertx(std::is_eq(compare_wi(wi, wi2)));
            if (diff) break;
...
void parallel_optimize() {
  ...
  for (;;) {
    ...
    for_int(index, ar_edgecost.num()) {
      ...
      assertw(ecol_result.vs == vs);
      assertw(ecol_result.cost == cost);  // Exact only without -ffast-math; see also -funsafe-math-optimizations.
      // assertw(abs(ecol_result.cost - cost) <= 1e-4f * cost);  // Else: relative, since cost scales as length^2.
    }
    if (verb >= 2)
