...
void print_principal(const Frame& f) {
    if (iob) {
        ...
        iob->diffuse(.6f, .6f, .6f); iob->specular(.5f, .5f, .5f); iob->phong(4.f);
        iob->push();
        iob->apply(f*xformi); draw_pc_extent(*iob);
        iob->pop();
    }
    if (iof) {
        iof->diffuse(1.f, 1.f, 1.f); iof->specular(0.f, 0.f, 0.f); iof->phong(1.f);
        iof->begin_force_polyline(true); {
            iof->push();
            iof->apply(f*xformi); draw_pc_linear(*iof);
            iof->pop();
        } iof->end_force_polyline();
    }
    if (iou) {
        iou->diffuse(.7f, .3f, .1f); iou->specular(.6f, .6f, .2f); iou->phong(3.f);
        iou->push();
        iou->apply(f*xformi); draw_pc_linear(*iou);
        iou->pop();
    }
}
...
void process_principal() {
    ...
    HH_STAT(Sr21); HH_STAT(Sr20); HH_STAT(Sr10);
    HH_STAT(Slen2); HH_STAT(Slen1); HH_STAT(Slen0);
    HH_STAT(Snei);
    for_int(i, num) {
        int n; Frame f; compute_tp(i, n, f);
        if (ioo) pctrans[i] = f;
        Snei.enter(n);
        float len0 = mag(f.v(0)), len1 = mag(f.v(1)), len2 = mag(f.v(2));
        assertx(len2>0);        // principal_components() should do this
        Slen0.enter(len0); Slen1.enter(len1); Slen2.enter(len2);
        Sr10.enter(len1/len0); Sr20.enter(len2/len0); Sr21.enter(len2/len1);
        pcorg[i] = f.p();
        pcnor[i] = usenormals<3 ? f.v(minora) : nor[i];
        assertx(pcnor[i].normalize());
        print_principal(f);
    }
    close_mk(iob); close_mk(iof); close_mk(iou);
    if (iod) {
        iod->diffuse(1.f, 1.f, .3f); iod->specular(0.f, 0.f, 0.f); iod->phong(1.f);
        for_int(i, num) {
...
void add_exterior_orientation(const Set<int>& nodes) {
    ...
    if (have_normals) {
        ...
        float maxz = -BIGFLOAT; int maxi = -1;
        for (int i : nodes) {
...
void remove_exterior_orientation() {
    Array<int> ari; for (int j : gpcpseudo->edges(num)) { ari.push(j); }
...
void show_propagation(int i, int j, float dotp) {
    ...
    iop->specular(0.f, 0.f, 0.f); iop->phong(3.f);
...
float pc_dist(int e1, int e2) {
    return dist(pcorg[e1], pcorg[e2]);
}

void draw_oriented_tps() {
    ioo->diffuse(.7f, .3f, .1f); ioo->specular(.6f, .6f, .2f); ioo->phong(3.f);
    for_int(i, num) {
        Frame& f = pctrans[i];
...
void orient_tp() {
    ...
    }
    {
        ...
        stat.set_name("gpcpseudo"); stat.set_print(true);
    }
    {
    ...
    if (iog) {
        iog->diffuse(.7f, .7f, .7f); iog->specular(0.f, 0.f, 0.f); iog->phong(1.f);
        print_graph(*iog, *gpcpseudo.get(), pcorg, nullptr);
        close_mk(iog);
    }
    // Now treat each connected component of gpcpseudo separately.
    Set<int> setnotvis; for_int(i, num) { setnotvis.enter(i); }
    ...
    close_mk(ioo);
    if (ioh) {
        ioh->diffuse(.7f, .7f, .7f); ioh->specular(0.f, 0.f, 0.f); ioh->phong(1.f);
...
void print_directed_seg(Mk3d& mk, const Point& p1, const Point& p2, const A3dColor& col) {
    Vector v = p2-p1;
    assertx(v.normalize());
    mk.diffuse(col); mk.specular(0.f, 0.f, 0.f); mk.phong(3.f);
    mk.point(p1*xformi);
    mk.normal(v);
    mk.point(p2*xformi);
    mk.normal(v);
    ...
    void operator()(const Array<Vec3<float>>& poly) const {
        ...
        for_int(i, poly.num()) {
            el.push(A3dVertex(Point(poly[i])*xformi, Vector(0.f, 0.f, 0.f),
                              A3dVertexColor(A3dColor(1.f, .16f, 0.f), A3dColor(1.f, .5f, .3f),
                                             A3dColor(3.f, 0.f, 0.f))));
        }
        ioc->oa3d().write(el);
    }
};
...
struct output_border2D {
    void operator()(const Array<Vec2<float>>& poly) const {
        assertx(ioc); ASSERTX(poly.num()==2);
...
template<typename Contour> void contour_2D(Contour& contour) { // with or without border
    ...
    if (unsigneddis) {
        ...
        for_int(i, num) { Point p = pcorg[i]; contour.march_from(V(p[1], p[2])); }
    }
}
