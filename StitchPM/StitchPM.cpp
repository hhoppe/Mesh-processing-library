...
int compute_nvi(int bx, int by, int vi) {
    ...
    if (vi<=blocks) {
        ...
        x = vi-(blocks+1); y = blocks;
    } else if (vi<=3*blocks) {
        x = 0; y = vi-(2*blocks+1);
    } else {
        x = blocks; y = vi-(3*blocks);
    }
    if (0) {
        // old scheme.
        if (0) ASSERTX((blocky+1)*(blockx*blocks+1) + (blockx+1)*((blocky*(blocks-1))) ==
        ...
        if (y==0 && by==0) {
            ...
            return (blockx*blocks+1)*2 + (blocky*blocks-1)*2 + (by+(y==blocks)-1)*(blockx*blocks-1) + bx*blocks+(x-1);
        } else if (x==0 || x==blocks) {
...
void do_stitch() {
    ...
    for_int(vi, bmesh._vertices.num()) {
        bmesh._wedges[vi].vertex = vi;
    }
    ...
    for_int(bx, blockx) {
        for_int(by, blocky) {
            ...
            for_int(vspli, pmxy._vsplits.num()) {
                ...
                for_int(count, vspl.adds_two_faces() ? 2 : 1) {
                    f_renumber[bx][by][pmxy_nfaces++] = pmesh_nfaces++;
                }
                pmesh_nvertices++;
            }
