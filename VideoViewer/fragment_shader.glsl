...
float keys(float x) {
  float r = abs(x);
  return 0.5f * (r < 1.0f ? 2.0f + r * r * (-5.0f + 3.0f * r) : r < 2.0f ? 4.0f + r * (-8.0f + (5.0f - r) * r) : 0.0f);
}
...
float mitchell(float x) {
  ...
  return (1.0f / 18.0f) * (r < 1.0f   ? 16.0f + r * r * (-36.0f + 21.0f * r)
                           : r < 2.0f ? 32.0f + r * (-60.0f + (36.0f - 7.0f * r) * r)
                                      : 0.0f);
}
...
float bspline3(float x) {
  ...
  return (r < 1.0f   ? 2.0f / 3.0f + r * r * (-1.0f + r * 0.5f)
          : r < 2.0f ? 4.0f / 3.0f + r * (-2.0f + r * (1.0f - r / 6.0f))
                     : 0.0f);
}
...
float omoms3(float x) {
  float r = abs(x);
  return 1.0f / 42.0f *
         (r < 1.0f   ? 26.0f + r * (3.0f + r * (-42.0f + r * 21.0f))
          : r < 2.0f ? 58.0f + r * (-85.0f + r * (42.0f + r * -7.0f))
                     : 0.0f);
}
...
vec4 RGB_to_YUV(vec4 rgb) {
  return vec4(rgb[0] * (vec3(66.f, -38.f, 112.f) / 256.f) + rgb[1] * (vec3(129.f, -74.f, -94.f) / 256.f) +
                  rgb[2] * (vec3(25.f, 112.f, -18.f) / 256.f) + (vec3(16.f, 128.f, 128.f) / 256.f),
              rgb[3]);
}

vec4 YUV_to_RGB(vec4 yuv) {
  return vec4(yuv[0] * (vec3(298.f, 298.f, 298.f) / 256.f) + yuv[1] * (vec3(0.f, -100.f, 516.f) / 256.f) +
                  yuv[2] * (vec3(409.f, -208.f, 0.f) / 256.f) +
                  (vec3(-298.f * 16.f - 409.f * 128.f, -298.f * 16.f + 100.f * 128.f + 208.f * 128.f,
                        -298.f * 16.f - 516.f * 128.f) /
                   65536.f),
              yuv[3]);
}
...
void main() {
  ...
  if (true) {  // Apply: OVER background color.
