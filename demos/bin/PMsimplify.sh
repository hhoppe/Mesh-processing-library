...
The edge collapses are recorded from the start (as the old vsplit records must continue from the old base mesh),
...
tname() { echo "${1##*/}"; }
ename() { local t="$(tname "$1")"; [[ $t = *.* ]] && echo "${t##*.}" || echo ''; }
rname() { echo "${1%.$(ename "$1")}"; }

inroot="$1"; shift
if [[ $(ename "$inroot") == gz ]]; then inroot="$(rname "$inroot")"; fi
if [[ $(ename "$inroot") != pm ]]; then echo "$0: first argument not *.pm" >&2; exit 1; fi
inroot="$(rname "$inroot")"

for arg in "$@"; do
  if [[ $arg == -prog* ]]; then echo "$0: the arguments must not include '$arg'" >&2; exit 1; fi
done
...
# Not '| gzip' because it would strip out all SHOWFF() output.
Filterprog -fbase "$tmpbase" -fprog "$tmpprog" -append_old_pm "$inroot.pm" -pm >"$outroot.pm"
