...
#undef T
    ...
    int bitmask = XWMGeometry(_display, _screen,
                              to_nullptr_or_cstring(_user_geometry), to_nullptr_or_cstring(_default_geometry),
                              1, pxsh, &pxsh->x, &pxsh->y,
                              &pxsh->width, &pxsh->height, &pxsh->win_gravity);
    ...
    _win_dims = V(pxsh->height, pxsh->width);
    {
        ...
        _win = XCreateWindow(_display, RootWindow(_display, _screen), pxsh->x, pxsh->y, pxsh->width, pxsh->height,
                             border_width, _depth, InputOutput,
    ...
    XSelectInput(_display, _win, ExposureMask | KeyPressMask | ButtonPressMask |
                 ButtonReleaseMask | StructureNotifyMask);
    if (0) XSelectInput(_display, _win, ExposureMask | KeyPressMask | ButtonPressMask | ButtonReleaseMask |
                        StructureNotifyMask | EnterWindowMask | LeaveWindowMask | FocusChangeMask |
                        PropertyChangeMask | VisibilityChangeMask | SubstructureNotifyMask);
    if (_oglx) {
    ...
    switch (_event.type) {
     ...
     case ButtonPress: case ButtonRelease: {
        if (_hwdebug) SHOW("Button");
         ...
         const bool shift = (_event.xbutton.state&ShiftMask)!=0; dummy_use(shift);
...
bool HW::got_event() {
    ...
    if (!_gotevent && XCheckMaskEvent(_display, 0xfffffff, &_event))
        _gotevent = true;
    return _gotevent;
}
...
void HW::set_double_buffering(bool newstate) {
    ...
    if (newstate!=ostate) {
        if (_async || _state!=EState::open) {
            assertx(!_need_toggle_buffering); _need_toggle_buffering = true;
        } else {
            toggle_buffering();
        }
    }
...
bool HW::get_key_modifier(EModifier modifier) {
    Vec<char,32> keys; XQueryKeymap(_display, keys.data());
...
void HW::set_color(const Pixel& pix) {
    ...
    if (_oglx) {
        ...
        XColor xcolor;
        xcolor.red = pix[0]*256; xcolor.green = pix[1]*256; xcolor.blue = pix[2]*256;
    ...
    if (_state==EState::open) {
        ...
        if (0) {
            ...
            XChangeProperty(_display, _win,
                            ...
                            8, PropModeReplace, reinterpret_cast<uchar*>(const_cast<char*>(_window_title.c_str())),
                            _window_title.size());
        }
        XFlush(_display);
    }
...
Vec2<int> HW::get_max_window_dims() {
    XWindowAttributes attribs; assertx(XGetWindowAttributes(_display, DefaultRootWindow(_display), &attribs));
...
void HW::make_fullscreen(bool b) {
    ...
    if (use_change_property) {
        ...
        if (b) {
            ...
            XChangeProperty(_display, _win, XInternAtom(_display, "_NET_WM_STATE", False),
                            XA_ATOM, 32, PropModeReplace, reinterpret_cast<uchar*>(atoms.data()), atoms.num());
...
void HW::wake_up() {
    ...
    if (0) {
        ...
        //   display* structures."
        XEvent event; std::memset(&event, 0, sizeof(event));
        event.type = Expose;
