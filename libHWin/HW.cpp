...
static BOOL CALLBACK find_win_by_procid(HWND hwnd, LPARAM lp) {
    dummy_use(lp);
    int pid; GetWindowThreadProcessId(hwnd, LPDWORD(&pid));
...
void HW::open() {
    ...
    Vec2<int> yxpos; {
        ...
        if (sscanf(s.c_str(), "%dx%d+%d+%d", &_win_dims[1], &_win_dims[0], &yxpos[1], &yxpos[0])==4) {
            ...
            yxpos = twice(-1); _win_dims = twice(500);
        }
    }
    ...
    if (_offscreen=="") {
        ...
        if (!_pbuffer) {
            ...
            for_int(c, 2) { while (!is_pow2(tdims[c])) tdims[c]++; }
            Image timage(tdims);
            GLenum internal_format = GL_RGBA8;
            glTexImage2D(GL_TEXTURE_2D, 0, internal_format, tdims[1], tdims[0], 0,
                         GL_RGBA, GL_UNSIGNED_BYTE, timage.data());
            int get_w; glGetTexLevelParameteriv(GL_TEXTURE_2D, 0, GL_TEXTURE_WIDTH, &get_w); assertx(get_w==tdims[1]);
...
bool HW::loop() {
    for (;;) {
        ...
        if (!got_event() && !_is_keyintr) {
            ...
            if (_watch_fd0) {
                DWORD ret = MsgWaitForMultipleObjectsEx(1, &g_buf_event_data_available, INFINITE, QS_ALLEVENTS,
                                                        MWMO_INPUTAVAILABLE);
    ...
    switch (iMsg) {
     ...
     case WM_SIZE: {
         ...
         if (0 && gl_extensions_string()!="") { // only if OpenGL is already initialized.
             ...
             bool bu_is_glx_dbuf = _is_glx_dbuf; _is_glx_dbuf = false;
             draw_it();
...
bool HW::got_event() {
    ...
    if (!_gotevent && PeekMessage(&_msg, nullptr, 0, 0, PM_REMOVE))
        _gotevent = true;
    return _gotevent;
}
...
void HW::start_hwkey() {
    ...
    _sk_timerID = timeSetEvent(timer_delay, _sk_timerResolution, callbackSimuKey,
...
void HW::end_hwkey() {
    ...
    if (_sk_timerID) { assertx(timeKillEvent(_sk_timerID)==TIMERR_NOERROR); _sk_timerID = 0; }
    if (_sk_timerResolution) { assertx(timeEndPeriod(_sk_timerResolution)==TIMERR_NOERROR); _sk_timerResolution = 0; }
}
...
Vec2<int> HW::get_max_window_dims() {
    RECT wa_rect; assertx(SystemParametersInfo(SPI_GETWORKAREA, 0, &wa_rect, 0));
    ...
    auto window_borders = top+V(2*(3+1+0), 2*(3+1+0)); // iBorderWidth default 1, +iPaddedBorderWidth (def 4, set 0)
...
void HW::resize_window(const Vec2<int>& yx) {
    ...
    if (_win_pos[0]+height>wa_rect.bottom ||
        _win_pos[1]+width >wa_rect.right) { new_top = wa_rect.top; new_left = wa_rect.left; force_move = true; }
...
void HW::set_pixel_format(bool fake_first) {
    ...
    if (1) {
        PIXELFORMATDESCRIPTOR pfd2; assertx(DescribePixelFormat(_hRenderDC, iPixelFormat, sizeof(pfd2), &pfd2));
        if (_hwdebug) {
            SHOW("Obtained:");
...
void HW::ogl_create_window(const Vec2<int>& yxpos) {
    ...
    _scr_stencilbits = getenv_int("STENCIL_BITS"); // dynamically updated by my_setenv() in G3dOGL.cpp
    {
        ...
        _hwnd = CreateWindowW(widen(_argv0).c_str(), widen(_window_title).c_str(), style,
    ...
    USE_GL_EXT_MAYBE(wglChoosePixelFormatARB,
                     BOOL (WINAPI*) (HDC hdc, const int* piAttribIList, const FLOAT* pfAttribFList,
                                     UINT nMaxFormats, int* piFormats, UINT* nNumFormats));
    DECLARE_HANDLE(HPBUFFERARB);
    USE_GL_EXT_MAYBE(wglCreatePbufferARB,
                     HPBUFFERARB (WINAPI*) (HDC hDC, int iPixelFormat, int iWidth, int iHeight,
    ...
    USE_GL_EXT_MAYBE(wglSwapIntervalEXT, BOOL (WINAPI*) (int interval));
    {
        ...
        assertx(wglMakeCurrent(_hRenderDC, nullptr));
        assertx(wglDeleteContext(_hRC)); _hRC = nullptr;
        _hRenderDC = nullptr;
        assertx(ReleaseDC(_hwnd, _hDC)); _hDC = nullptr;
        assertx(DestroyWindow(_hwnd)); _hwnd = nullptr;
    }
    ...
    // 0=none, 2=2samples, 3=2+5tap(Qunicux), 4=4samples, 5=4+9tap
    ...
    if (_offscreen!="" && !_pbuffer) {
        ...
        _bitmap = assertx(CreateDIBSection(_hDC, reinterpret_cast<BITMAPINFO*>(&bmih),
                                           DIB_RGB_COLORS, &_bitmap_data, nullptr, 0));
    ...
    if (_pbuffer || _multisample) {
        ...
        for (;;) {
            ...
            assertx(wglChoosePixelFormatARB(_hRenderDC, iattribl.data(), fattribl.data(),
                                            unsigned(iFormats.num()), iFormats.data(), &numFormats));
    ...
    //                  values of a matrix!)
    //               -a GLRC can only be used by one thread (ie each thread
    //                  must have its own)
    //               -BUT you can associate multiple GLRCs with the same
    //                  device context
    ...
    {
        TEXTMETRIC tm; assertx(GetTextMetrics(_hRenderDC, &tm));
    ...
    if (getenv("SWAP_INTERVAL")) {
        ...
        if (wglSwapIntervalEXT)
            assertw(wglSwapIntervalEXT(interval));
    }
