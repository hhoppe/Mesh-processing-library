...
inline void HWbase::handle_keyintr() {
    ...
    int is_group = 0;
    for (;;) {
        const char ch = _hwkey[0]; _hwkey.erase(0, 1);
...
#define USE_GL_EXT_MAYBE_AUX(func, type, GetProc)                               \
    ...
    if (!is_init_##func) { is_init_##func = true; func = Type##func(GetProc); } \
    HH_EAT_SEMICOLON

#if defined(_WIN32)
#define USE_GL_EXT_MAYBE(func, type) USE_GL_EXT_MAYBE_AUX(func, type, wglGetProcAddress(#func))
#elif 1                         // Unix / X windows
#define USE_GL_EXT_MAYBE(func, type) USE_GL_EXT_MAYBE_AUX(func, type,         \
                                                          glXGetProcAddress(reinterpret_cast<const GLubyte*>(#func)))
#else  // Mac OSX
#define USE_GL_EXT_MAYBE(func, type) USE_GL_EXT_MAYBE_AUX(func, type, NSGLGetProcAddress(#func))
#endif

// See ~/src/G3dOGL/glext.h
#define USE_GL_EXT(func, type) USE_GL_EXT_MAYBE(func, type); assertx(func)
...
inline const string& gl_extensions_string() {
    ...
    if (!is_init) {
#if 0
        int n; glGetIntegerv(GL_NUM_EXTENSIONS, &n);
        ...
        if (!s) {
            ...
            s_string = s; s_string += " "; is_init = true;
        }
#endif
    }
    return s_string;
