    ...
    if (1 && o.doc!="") {       // enforce my "conventions" to properly distinguish flags and parameters
        if (o.narg>0) {
            if (o.doc[0]==':' || !contains(o.doc, ':') || o.doc[o.doc.find(':')-1]!=' ') {
                SHOW(o.str, o.doc); Warning("Possibly inconsistent parameter option comment");
            }
        }
        if (!o.narg && o.parsef) {
            if (o.doc[0]!=':') {
                SHOW(o.str, o.doc); Warning("Possibly inconsistent flag option comment");
            }
        }
    }
...
auto ParseArgs::match(const string& s, bool skip_options) -> const option* {
    ...
    for (option& o : _aroptions) {
        ...
        if (!o.str.compare(0, nchar, s, 0, nchar)) {
            ...
            if (!omatch || lo<minlfound) { minlfound = lo; omatch = &o; }
        }
    }
    if (!minlfound) nmatches = 1;
    if (nmatches>1)
        showf("Argument '%s' is ambiguous, '%s' assumed\n", s.c_str(), assertx(omatch)->str.c_str());
    return omatch;
}
...
bool ParseArgs::parse_internal() {
    ...
    while (num()) {
        ...
        if (arg=="--") {
            ...
            if (!_other_options_ok) { shift_args(); continue; }
        } else {
        ...
        if (!_curopt) {         // not found
            if ((_other_args_ok && !is_option) ||
                (_other_options_ok && is_option)) {
...
void ParseArgs::fbool(Args& args) {
    ...
    if (!n) {                   // set a flag variable
        if (0 && *argp) { SHOW(pargs._curopt->str); Warning("ParseArgs: flag is already set"); }
        *argp = true;
