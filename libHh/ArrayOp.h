...
template<typename R, typename = enable_if_range_t<R> > Vec2<iterator_t<R> > median2(const R& range) {
    using T = iterator_t<R>;
    using std::begin; using std::end;
    Array<T> ar(begin(range), end(range));
    assertx(ar.num());
...
template<typename R, typename = enable_if_range_t<R> > mean_type_t<iterator_t<R> > median(const R& range) {
    return mean(median2(range));
}

// Return the element with specified rank within range (where 0 <= rank < size(range) and rank==0 is minimum element).
