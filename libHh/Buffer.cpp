...
#if defined(BUFFER_USE_WIN32_THREAD)
    if (_fd==0) {
        if (WaitForSingleObject(g_buf_event_data_available, 0)==WAIT_TIMEOUT)
            return ERefill::no;
...
void WBuffer::put(const void* buf, int nbytes) {
    assertx(!eof() && !err());
    if (_beg+_n+nbytes>_ar.num() && _beg)
        shift();
    while (_n+nbytes>_ar.num())
        expand();
...
void WBuffer::put(char ch) {
    put(&ch, 1);
}

void WBuffer::put(short i) {
    short t = i; to_std(&t); put(&t, 2);
}

void WBuffer::put(int i) {
    int t = i; to_std(&t); put(&t, 4);
}

void WBuffer::put(float f) {
    float t = f; to_std(&t); put(&t, 4);
}
