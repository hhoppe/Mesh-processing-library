...
template<typename T> class MoveToFront : noncopyable {
 public:
    int enter(const T& e) {
        ...
        if (ifound<0) { ifound = _list.num(); _list.push(e); }
        //  (alternative faster implementation would be to randomly swap with another element closer to front,
...
template<typename T> class Encoding : noncopyable {
 public:
    Encoding() {
        ...
        assertx(!movetofront);
        // No longer supported because would only work if _map was Map<int,float>, i.e. for T==int.
        // Instead, perform MovetoFront outside this class.
    }
    void add(const T& e, float prob) {
        // if (_movetofront) i = _movetofront->enter(e);
        bool is_new; float& oprob = _map.enter(e, prob, is_new);
        if (!is_new) oprob += prob;
    }
    void print() const {
        struct P { float prob; string str; };
        Array<P> ar; ar.reserve(_map.num());
    ...
    float huffman_cost() const {
        Pqueue<int> pq; pq.reserve(_map.num());
        ...
        for (;;) {
            if (pq.num()<2) break;
            float prob1 = pq.min_priority(); pq.remove_min();
            float prob2 = pq.min_priority(); pq.remove_min();
    ...
    float worst_entropy() const {
        ...
        // For arithmetic coding with large probability distributions, the
        //  distribution itself is very predictable.  What requires encoding is the
        //  permutation of the symbols in the sorted decreasing list of symbols.
        // This permutation requires approximately m*log(m) bits if there are m
        //  symbols.
    ...
    void print_top_entries(const string& name, int ntop, Func cb_entry_name) const {
        ...
        if (0) {
            ...
            float max_prob = 0.f; for (float prob : _map.values()) { max_prob = max(max_prob, prob); }
            ...
            for_int(i, min(ntop, ar.num())) {
                const P& p = ar[i];
                float prob = p.prob; string ename = p.str;
                cumu_prob += prob;
                showdf("  %20s  %.5f  cumu %.5f\n", ename.c_str(), prob/tot_prob, cumu_prob/tot_prob);
            }
        }
...
class DeltaEncoding {
 public:
    ...
    int analyze(const string& s) const {
        ...
        if (!s.empty())
            showdf("DE %s: n=%d  nbits=%.1f  sign=%.1f  bdelta=%.1f  total=%.1f  (%d)\n",
                   s.c_str(), _num,
                   _enc_nbits.entropy()/_num, enc_signs/_num,
                   static_cast<float>(_delta_bits)/_num, static_cast<float>(total_bits)/_num,
                   total_bits);
        return total_bits;
    }
    static int val_bits(float v) {
        float a = abs(v);
        if (a<1.f) return 0;
        // int vbits = static_cast<int>(ceil(log2(a))); // old method
        int vbits = static_cast<int>(floor(log2(a+1.0001f))); // new method 970609
 ...
 private:
    ...
    int _prev_sign {0};
    Vec2<Encoding<int>> _enc_sign; // based on previous sign
};
