...
// Also defines std::ostream operators for std::tuple<>.   (ostream for std::pair<> is in Hh.h)

namespace std {
template<typename T1, typename T2> struct hash<std::pair<T1,T2>> {
    size_t operator()(const std::pair<T1,T2>& p) const {
        return ::hh::hash_combine(::hh::my_hash(p.first), p.second);
    }
};
} // namespace std
...
template<typename TU> struct tuple_hash<TU,0> {
    size_t operator()(const TU& tu) const {
        return hh::my_hash(std::get<0>(tu));
    }
};
} // namespace details
} // namespace hh
