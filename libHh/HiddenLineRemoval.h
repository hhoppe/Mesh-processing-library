...
class HiddenLineRemoval {
 ...
 public:
  ...
  bool draw_point(const Point& p) { return draw_point_i(p); }             // Returns true if the point is visible.
  void set_draw_seg_cb(draw_seg_type func) { _func_draw_seg_cb = func; }  // Could be templated.
  void draw_segment(const Point& p1, const Point& p2) { draw_segment_i(p1, p2); }
 ...
 private:
  ...
  KD _kd{8};
  static constexpr int k_max_intersections = 20;  // Maximum number of times a polygon can split a segment.
  ...
  void enter_i(Polygon poly) {
    ...
    hp.nor = nor;
    hp.d = hp.poly.get_planec(hp.nor);
    // Added float{} cast to avoid const reference of constexpr.
    hp.tol = max(hp.poly.get_tolerance(hp.nor, hp.d), float{k_epsilon_a}) * 1.02f;
    hp.bbox = Bbox{hp.poly};
    // Enter coordinates [1..2] into Kdtree<int, 2>.
    _kd.enter(pn, hp.bbox[0].tail<2>(), hp.bbox[1].tail<2>());
  }
  ...
  bool draw_point_i(const Point& p) {
    ...
    const auto func_hlr_point_consider_poly = [&](const int& pn, ArrayView<float>, ArrayView<float>, KD::CBloc) {
      ...
      if (p[0] < hp.bbox[0][0]) return KD::ECallbackReturn::nothing;  // Point in front of the bbox of the polygon.
      const Polygon& poly = hp.poly;
      const float d = dot(p, hp.nor) - hp.d;
      if (d <= hp.tol) return KD::ECallbackReturn::nothing;  // Point in front of the plane of the polygon.
  ...
  void draw_segment_i(const Point& p1, const Point& p2) {
    HlrSegment s(p1, p2);
    render_seg_kd(s, 0);
  }
  ...
  void render_seg_kd(HlrSegment& s, KD::CBloc kdloc) {
    ...
    const auto func_hlr_seg_consider_poly = [&](const int& pn, ArrayView<float> bb0, ArrayView<float> bb1,
      ...
      if (ret == KD::ECallbackReturn::bbshrunk) {
        ...
        bb0[0] = min(s.p[0][1], s.p[1][1]);
        bb1[0] = max(s.p[0][1], s.p[1][1]);
        bb0[1] = min(s.p[0][2], s.p[1][2]);
        bb1[1] = max(s.p[0][2], s.p[1][2]);
      }
      return ret;
    };
  ...
  static bool intersect_seg_side(HlrSegment& s, const Point& p0, const Point& p1, HlrSegment& news) {
    ...
    if (!f) return false;  // Collinear segments.
  ...
  int intersect_seg_poly(const HlrSegment& s, const HlrPolygon& hp) {
    ...
    int no = 0;  // Number of segments in the subset of ni that are visible.
    _gsa[0] = s;
    const Polygon& poly = hp.poly;
    ...
    for_int(i, pnum) {
      ...
      for_int(j, ni) {
        ...
        if (intersect_seg_side(_gsa[j], poly[il], poly[i], _gsa[ni])) assertx(++ni < k_max_intersections);
      }
      il = i;
      y0 = y1;
      z0 = z1;
  ...
  int join_set_and_seg(HlrSegment& s, int nsi) {
    if (nsi == 1) {  // Only handle the common case.
  ...
  // Returns nothing = no_effect, bbshrunk = continue_with_changed_seg, stop = seg_gone.
  KD::ECallbackReturn handle_polygon(HlrSegment& s, int pn, KD::CBloc kdloc) {
    ...
    int ns;                       // Number of resulting segments.
    if (d1 > tol && d0 < -tol) {  // Point 1 is back.
      ...
      // Try to reunite the set _gsret and the segment front_s.
      ns = join_set_and_seg(front_s, ns);
    } else {
      ns = intersect_seg_poly(s, hp);
    }
    if (!ns) return KD::ECallbackReturn::stop;  // The segment is gone.
    s = _gsret[0];
    --ns;
