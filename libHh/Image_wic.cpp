...
#include <windows.h>            // required by WIC
...
struct PSG { string suffix; const GUID* guid; };
const Array<PSG> k_ar_suffix_container = {
    PSG{"bmp",  &GUID_ContainerFormatBmp},
    PSG{"png",  &GUID_ContainerFormatPng},
    PSG{"jpg",  &GUID_ContainerFormatJpeg},
    PSG{"jpeg", &GUID_ContainerFormatJpeg},
    PSG{"tif",  &GUID_ContainerFormatTiff},
    PSG{"tiff", &GUID_ContainerFormatTiff},
    PSG{"gif",  &GUID_ContainerFormatGif},
    PSG{"wmp",  &GUID_ContainerFormatWmp},
};

const GUID* get_container_format(const string& suffix) {
    for (auto& p : k_ar_suffix_container) { if (p.suffix==suffix) return p.guid; }
    return nullptr;
}
...
void Image::read_file_wic(const string& filename, bool bgra) {
    ...
    if (file_requires_pipe(filename)) {
        ...
        success = SUCCEEDED(wic_factory->CreateDecoderFromFilename(widen(filename).c_str(), nullptr,
    ...
    {
        com_ptr<IWICBitmapFrameDecode> frame_decode; AS(decoder->GetFrame(0, &frame_decode));
        // HOW(RefCount(frame_decode)); // note: it is 2 because also referenced by decoder
        unsigned width = 0, height = 0; AS(frame_decode->GetSize(&width, &height));
        init(V(int(height), int(width)));
        // double dpi_X = 0., dpi_Y = 0.; AS(frame_decode->GetResolution(&dpi_X, &dpi_Y));
        WICPixelFormatGUID pixel_format; my_zero(pixel_format); AS(frame_decode->GetPixelFormat(&pixel_format));
        set_zsize(k_pixel_formats_with_alpha.contains(pixel_format) ? 4 : 3); // ignore grayscale for now
        {
            ...
            if (SUCCEEDED(frame_decode->GetMetadataQueryReader(&pQueryReader))) {
                PROPVARIANT propvariant; PropVariantInit(&propvariant);
        ...
        com_ptr<IWICFormatConverter> converter; AS(wic_factory->CreateFormatConverter(&converter));
        ...
        if (1) {                    // verify the pixel format
            WICPixelFormatGUID pixel_format3; AS(converter->GetPixelFormat(&pixel_format3));
    ...
    if (orientation==1) {
        ...
        struct S { int angle; bool fliph; bool flipv; };
        const Array<S> ar = {
...
void Image::write_file_wic(const string& filename, bool bgra) const {
    ...
    if (write_through_memory) {
        ...
        com_ptr<IWICStream> output_wic_stream; AS(wic_factory->CreateStream(&output_wic_stream));
    ...
    com_ptr<IWICBitmapEncoder> encoder; AS(wic_factory->CreateEncoder(*container_format, nullptr, &encoder));
    ...
    // WIC GUIDs and CLSIDs:  http://msdn.microsoft.com/en-us/library/windows/desktop/ee719882%28v=vs.85%29.aspx
    {
        com_ptr<IWICBitmapFrameEncode> frame_encode; {
            ...
            if (container_format==&GUID_ContainerFormatJpeg) {
                ...
                PROPBAG2 option; my_zero(option); option.pstrName = const_cast<wchar_t*>(L"ImageQuality");
        ...
        if (have_metadata) {
            const WICDecodeOptions opts = WICDecodeMetadataCacheOnDemand;
            if (!SUCCEEDED(wic_factory->CreateDecoderFromFilename(widen(orig_filename).c_str(), nullptr,
                                                                  GENERIC_READ, opts, &decoder))) {
                SHOW(orig_filename, filename); Warning("Could not decode original image metadata");
            } else {
        ...
        WICPixelFormatGUID pixel_format = (zsize()==4 ? GUID_WICPixelFormat32bppBGRA :
        ...
        com_ptr<IWICBitmap> bitmap; {
            ...
            AS(wic_factory->CreateBitmapFromMemory(xsize(), ysize(), bitmap_pixel_format,
