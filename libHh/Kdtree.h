...
template<typename T, int D> class Kdtree : noncopyable {
 ...
 public:
    ...
    ~Kdtree()                                   { clear(); }
    void clear()                                { clear_i(); }
    void allow_duplication(float fsize)         { if (!getenv("KD_FSIZE")) _fsize = fsize; }
 ...
 private:
    ...
    struct Entry {
        ...
        Entry& operator=(Entry&& e) noexcept    { _id = std::move(e._id); _bb = e._bb; return *this; }
        T _id;
    ...
    void clear_i() {
        if (getenv_bool("KD_STATS") && _arnode.num()) { HH_STAT(SKDdepth); rec_depth(0, SKDdepth, 0); }
        _arentry.clear();
        _arnode.clear();
    }
    ...
    void enter_i(const T& id, const Vec<float,D>& bb0, const Vec<float,D>& bb1) {
        _arentry.push(Entry(id)); enter_aux(bb0, bb1);
    }
    void enter_i(T&& id, const Vec<float,D>& bb0, const Vec<float,D>& bb1) {
        _arentry.push(Entry(std::move(id))); enter_aux(bb0, bb1);
    }
    void enter_aux(const Vec<float,D>& bb0, const Vec<float,D>& bb1) {
        int ei = _arentry.num()-1;
        Entry& e = _arentry[ei];
        e._bb[0] = bb0; e._bb[1] = bb1;
        Vec<float,D> aval; fill(aval, .5f);
        float avgel = 0.f; if (_fsize) { for_int(i, D) { avgel += e._bb[1][i]-e._bb[0][i]; } avgel /= D; }
    ...
    void rec_enter(int ei, int ni, Vec<float,D>& aval, int level, float inc, int axis, float avgel) {
        ...
        for (;;) {
            ...
            if (want_l && want_h) { // single recursion
                ...
                if (_arnode[ni]._l<0) _arnode[ni]._l = _arnode.num();
                ni = _arnode[ni]._l; aval[axis] -= inc;
            } else if (want_h) {
                if (_arnode[ni]._h<0) _arnode[ni]._h = _arnode.num();
                ni = _arnode[ni]._h; aval[axis] += inc;
            } else assertnever("");
    ...
    template<typename Func> bool rec_search(int ni, int nlca, Vec<float,D>& bb0, Vec<float,D>& bb1,
                                            Func cbfunc, int& nelemvis) const {
        for (;;) {
            const Node& n = _arnode[ni];
            for (int ei : n._stackei) {
                ...
                for_int(i, D) {
                    ...
                    if (e._bb[0][i]>=bb1[i] || e._bb[1][i]<=bb0[i]) { overlaps = false; break; }
                }
                if (!overlaps) continue;
                if (cbfunc(e._id, bb0, bb1, nlca)==ECallbackReturn::stop)
                    return true;
            }
            const int axis = n._axis; ASSERTX(axis>=0 && axis<D);
