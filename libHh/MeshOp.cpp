...
int retriangulate(GMesh& mesh, SetEdge& sete, bool recurse, Set<Vertex>* setvr,
                  float mincos, EDGEF fdoswap, EDGEF fdel, EDGEF fadd) {
    assertx(fdoswap);
    int neswapped = 0;
    while (!sete.empty()) {
        ...
        for (Face f : mesh.faces(ne)) {
            for (Edge ee : mesh.edges(f)) {
                if (ee==ne || mesh.is_boundary(ee) ||
                    (setvr && (!setvr->contains(mesh.side_vertex1(ee)) ||
                               !setvr->contains(mesh.side_vertex2(ee)))))
                    continue;
                sete.add(ee);
            }
        }
...
float mesh_genus(const Mesh& mesh) {
    ...
    Stat Sbound = mesh_stat_boundaries(mesh); int nb = Sbound.inum();
    Stat Scompf = mesh_stat_components(mesh); int nc = Scompf.inum();
...
bool triangulate_face(GMesh& mesh, Face f) {
    Array<Vertex> va; mesh.get_vertices(f, va);
    int nv = va.num();
...
float edge_dihedral_angle_cos(const GMesh& mesh, Edge e) {
    ...
    if (mesh.is_triangle(f1) && mesh.is_triangle(f2)) {
        return dihedral_angle_cos(mesh.point(v1), mesh.point(v2),
                                  mesh.point(mesh.side_vertex1(e)),
                                  mesh.point(mesh.side_vertex2(e)));
    } else {
        return dihedral_angle_cos(mesh.point(v1), mesh.point(v2),
                                  interp(mesh.point(mesh.ccw_vertex(f1, v2)),
                                         mesh.point(mesh.clw_vertex(f1, v1))),
                                  interp(mesh.point(mesh.ccw_vertex(f2, v1)),
                                         mesh.point(mesh.clw_vertex(f2, v2))));
    }
}
...
float collapse_edge_inscribed_criterion(const GMesh& mesh, Edge e) {
    ...
    for (Vertex v : mesh.vertices(e)) {
        for (Face f : mesh.faces(v)) {
            ...
            if (dot(ar_normals[nnor], poly.get_normal_dir())<0)
...
float collapse_edge_volume_criterion(const GMesh& mesh, Edge e) {
    ...
    for (Vertex v : mesh.vertices(e)) {
        for (Face f : mesh.faces(v)) {
            ...
            if (dot(ar_normals[nnor], poly.get_normal_dir())<0.f)
                return BIGFLOAT; // flipped normal
            nnor++;
        }
    }
...
Set<Face> mesh_remove_boundary(Mesh& mesh, Edge erep) {
    ...
    while (!queuee.empty()) {
        Queue<Edge> qc; {
            Set<Vertex> setv; Vertex v = nullptr;
            for (Edge e : queuee) {
...
void Vnors::compute(const GMesh& mesh, Vertex v, EType nortype) {
    ...
    std::call_once(flag, [] {
        default_nor_type = (getenv_bool("SUM_NOR") ? EType::sum :
                            ...
                            EType::unspecified);
        if (assertw(default_nor_type==EType::unspecified))
            default_nor_type = EType::angle;
    });
    ...
    if (!nfaces) {
        ...
        clear(); _nor = Vector(0.f, 0.f, 0.f); return;
    }
    if (_mfnor && _mfnor->num()!=nfaces) clear();
    for (Edge e : mesh.edges(v)) { if (sharp(mesh, v, e)) nsharpe++; }
    ...
    for (Face frep : mesh.faces(v)) {
        ...
        for (;;) {              // find f: most_clw, or frep if closed
            Edge e = mesh.ccw_edge(f, v); if (sharp(mesh, v, e)) break;
            f = mesh.opp_face(f, e); if (f==frep) { closed = true; break; }
        }
        for (;;) {              // now go ccw
            ...
            if (is_cusp && nsharpe<2) {
                closed = false; av.push(mesh.opp_vertex(v, e)); break;
            }
            ...
            f = mesh.opp_face(f, e); if (closed && f==frep) break;
        }
        dummy_use(closed);
        int avn = av.num();
        ...
        if (nortype==EType::angle) {
            ...
            if (avn>2) {        // from Polygon::get_normal_dir()
                ...
                for_int(i, avn-1) {
                    pnor += cross(vp, mesh.point(av[i]), mesh.point(av[i+1]));
                }
                if (dot(vec, pnor)<0) { Warning("flipnor_corner"); vec = -vec; }
            }
            ...
            {                   // direction could be wrong (very bad case)
                ...
                if (dot(vec, pnor)<0) { Warning("flipnor_regcrease"); vec = -vec; }
            }
            ...
            if (avn==2) {       // 2, -1, -1
                ...
                for_intL(i, 1, avn-1) {
                    vec += mesh.point(av[i])*(fcom*Trig::sin(i, idenom));
                }
            }
            vec = cross(v1, vec);
            if (avn>2) {
                ...
                if (dot(vec, pnor)<0) { Warning("flipnor_crease"); vec = -vec; }
            }
        } else if (avn==2) {
            ...
            Vector t2 = (mesh.point(mesh.clw_vertex(av[1], v))-
                         mesh.point(mesh.clw_vertex(av[0], v)));
            vec = cross(t1, t2);
...
float project_point_neighb(const GMesh& mesh, const Point& p, Face& pf, Bary& ret_bary, Point& ret_clp, bool fast) {
    ...
    Set<Face> setfvis;
    Polygon poly; mesh.polygon(pf, poly);
    Bary minbary;
    ...
    bool projquick = pfsmooth && !nearedge; HH_SSTAT(Sprojquick, projquick);
    if (projquick) {
        ret_bary = minbary;
        return mind2;
    }
    ...
    for (; ; ni++) {
        ...
        Set<Face> setf;
        Array<Vertex> va; mesh.get_vertices(pf, va); assertx(va.num()==3);
        for_int(j, 3) {
        ...
        struct S { Face f; float d2; };
        Array<S> ar; ar.reserve(setf.num());
        for (Face f : setf) {
            mesh.polygon(f, poly);
    ...
    HH_SSTAT(Sprojnei, ni); HH_SSTAT(Sprojf, nvis);
