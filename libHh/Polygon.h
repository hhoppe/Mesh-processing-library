...
class Polygon : public PArray<Point, 4> {
 ...
 public:
  ...
  [[nodiscard]] bool point_inside(const Vector& pnor, const Point& point) const;
...
[[nodiscard]] Array<Point> intersect_poly_poly(const Polygon& p1, const Polygon& p2);
// If there is a compiler error on the above line, then it is likely that '#include <Windows.h>' invoked
// '#include <wingdi.h>' which defined Polygon().
...
[[nodiscard]] std::optional<Point> intersect_plane_segment(const Vector& normal, float d, const Point& p1,
                                                           const Point& p2);

// Find a vector perpendicular to v (any such vector).
[[nodiscard]] Vector orthogonal_vector(const Vector& v);
