...
#define HH_POOL_ALLOCATION_3(T)                                               \
    static hh::Pool pool;                                                     \
    struct PoolInit {                                                         \
        PoolInit() { if (!count++) pool.construct(#T, 0, 0); }                \
        ~PoolInit() { if (!--count) pool.destroy(); }                         \
        static int count;                                                     \
    }
...
#define HH_INITIALIZE_POOL_NESTED(T, name)                                    \
    static T::PoolInit pool_init_##name
...
class Pool : noncopyable {
 public:
    ...
    void construct(const char* name, unsigned esize, int ealign) {
        ...
        if (getenv_int("POOL_DEBUG")>=2)
            showf("Pool %-20s: construct (size=%2d, align=%2d)\n", _name, _esize, _ealign);
        if (_esize) init();
    }
    void destroy() {
        assertx(_name);
        ...
        _name = nullptr; _esize = 0; _h = nullptr; _nalloc = 0; _chunkh = nullptr;
    }
 ...
 private:
    ...
    void grow() {
        ...
        assertx((uintptr_t(p)%_ealign)==0);
        char* l = p+(nelem-1)*_esize;
        for (; p<l; p += _esize)
            reinterpret_cast<Link*>(p)->next = reinterpret_cast<Link*>(p+_esize);
        reinterpret_cast<Link*>(p)->next = nullptr;
        _nalloc += nelem;
    }
    void grow_size(int size, int align) {
        assertx(!_h);
        if (!_esize) { _esize = unsigned(size); _ealign = align; init(); }
        grow();
    }
};
