...
void SRViewParams::set_frame(const Frame& frame) {
    _frame = frame;
}

void SRViewParams::set_zooms(const Vec2<float>& zoomyx) {
    _zoomyx = zoomyx;
}

void SRViewParams::activate_lr_planes(bool flag) {
    _activate_lr_planes = flag;
}

void SRViewParams::activate_bottom_plane(bool flag) {
    _activate_bottom_plane = flag;
}

void SRViewParams::activate_top_plane(bool flag) {
    _activate_top_plane = flag;
}

void SRViewParams::set_hither(float hither) {
    _hither = hither;
}

void SRViewParams::set_yonder(float yonder) {
    _yonder = yonder;
}

void SRViewParams::set_screen_thresh(float screen_thresh) {
    _screen_thresh = screen_thresh;
}

bool SRViewParams::ok() const {
    return _frame[0][0]!=BIGFLOAT && min(_zoomyx)>0.f;
}
...
inline const SRVertex* SRMesh::get_vt(int vspli) const {
    ASSERTX(_vsplits.ok(vspli)); return _quick_first_vt+vspli*2;
}
...
inline const SRFace* SRMesh::get_fl(int vspli) const {
    ASSERTX(_vsplits.ok(vspli)); return _quick_first_fl+vspli*2;
}
...
inline bool SRMesh::is_active_f(const SRFace* f) const {
    return f->aface!=&_isolated_aface;
}
...
inline bool SRMesh::creates_2faces(const SRVsplit* vspl) const {
    return vspl->fn[3]!=&_isolated_face;
}
    ...
    if (1) {
        ...
        {                       // to avoid ~EList(): not empty
            for_int(vi, _base_vertices.num()) {
                _vertices[vi].avertex->activev.unlink();
            }
            assertx(_active_vertices.empty());
            for_int(fi, _base_faces.num()) {
                _faces[fi].aface->activef.unlink();
            }
        ...
        for (EListNode* n = ndelim->next(); n!=ndelim; ) {
            ...
            for (SRVertex* v = va->vertex; ; ) {
                ...
                if (!is_left_child) {
                    ...
                    SRFace* f = get_fl(vspli); ASSERTX(is_active_f(f));
                    SRAFace* fa = f->aface;
...
bool SRMesh::vspl_legal(const SRVertex* vs) const {
    ...
    if ((!is_active_f(vspl->fn[0]) && vspl->fn[0]!=&_isolated_face) ||
        !is_active_f(vspl->fn[1]) ||
...
bool SRMesh::ecol_legal(const SRVertex* vt) const {
    ...
    ASSERTX(creates_2faces(vspl) || (!is_active_f(fr) &&
                                     vspl->fn[2]==&_isolated_face && vspl->fn[3]==&_isolated_face));
    if (fra->fnei[2]!=vspl->fn[2]->aface || fra->fnei[0]!=vspl->fn[3]->aface) return false;
    {
        ...
        ASSERTX(!creates_2faces(vspl) ||
                (fra->vertices[0]==(vt+0)->avertex &&
                 fra->vertices[2]==(vt+1)->avertex &&
...
void SRMesh::display_hierarchy_height() const {
    ...
    for_int(vi, _vertices.num()) {
        ...
        ar_height[vi] = height;
        if (k_debug) if (vp) assertx(vp<vs);
...
void SRMesh::read_pm(PMeshRStream& pmrs) {
    ...
    for_int(vspli, tot_nvsplits) {
        ...
        for_int(i, 2) {
            ...
            if (!i || cr2faces)
                f_pm2sr.push(_base_faces.num()+2*vspli+i);
        }
    }
    ...
    _bbox.clear(); for_int(vi, _vertices.num()) { _bbox.union_with(vgeoms[vi].point); }
...
void SRMesh::compute_bspheres(CArrayView<SRVertexGeometry> vgeoms) {
    ...
    // Compute bound(star(v)) of vertices in fully refined mesh.
    {
        ...
        // Unsquare the radii.
        for_int(vi, _vertices.num()) {
            if (is_active_v(&_vertices[vi]))
                ar_bsphere[vi].radius = sqrt(ar_bsphere[vi].radius);
        }
    }
...
void SRMesh::compute_nspheres(CArrayView<SRVertexGeometry> vgeoms) {
    ...
    // Compute bound(star(v)) of vertices in fully refined mesh.
    {
        ...
        for_int(fi, _faces.num()) {
            ...
            Vector fnormal = cross(p0, p1, p2); assertw(fnormal.normalize());
        ...
        for_int(vi, _vertices.num()) {
            if (is_active_v(&_vertices[vi]))
                ar_nsphere[vi].radius = sqrt(ar_nsphere[vi].radius);
        }
    }
    // Iterate back over vsplits to assign bounding spheres to
    //  interior vertices.
    ...
    for_int(vi, _vertices.num()) {
        ...
        Vector nor_b = dir_b; assertw(nor_b.normalize());
...
void SRMesh::write_srm(std::ostream& os) const {
    ...
    // Write out sizes.
    os << "SRM\n";
    os << "base_nvertices=" << _base_vertices.num() <<
        " base_nfaces=" << _base_faces.num() <<
    ...
    for_int(fi, _base_faces.num()) {
        ...
        for_int(j, 3) {
            write_binary_std(os, ArView(narrow_cast<int>(fa->vertices[j]-_base_vertices.data())));
        }
        for_int(j, 3) {
...
void SRMesh::read_srm(std::istream& is) {
    ...
    // Read in bounding box.
    {
        Bbox& bb = _bbox;
        string sline; assertx(my_getline(is, sline));
        assertx(sscanf(sline.c_str(), "bbox %g %g %g  %g %g %g",
                       &bb[0][0], &bb[0][1], &bb[0][2], &bb[1][0], &bb[1][1], &bb[1][2])==6);
    }
    // Read in materials.
    _materials.read(is);
    // Read in base mesh.
    ...
    for_int(fi, _base_faces.num()) {
        ...
        _faces[fi].aface = fa;
        fa->activef.link_before(_active_faces.delim());
        Vec3<int> vi; assertx(read_binary_std(is, vi.view()));
        Vec3<int> fni; assertx(read_binary_std(is, fni.view()));
        int matid; assertx(read_binary_std(is, ArView(matid)));
    ...
    for_int(vspli, _vsplits.num()) {
        ...
        srm_vsplit_binary_buf buf; assertx(read_binary_raw(is, ArView(buf)));
        from_std(&buf.vsi); int vsi = buf.vsi;
        SRVertex* vs = &_vertices[vsi];
        vs->vspli = vspli;
        for_int(j, 4) {
            from_std(&buf.fni[j]); int fni = buf.fni[j];
            vspl->fn[j] = fni ? &_faces[fni-1] : &_isolated_face;
        }
        from_std(&buf.fl_matid); short fl_matid = buf.fl_matid;
        from_std(&buf.fr_matid); short fr_matid = buf.fr_matid;
#if !defined(SR_PREDICT_MATID)
        vspl->fl_matid = fl_matid; vspl->fr_matid = fr_matid;
#else
        if (fl_matid || fr_matid) {
            static int warning_matid;
            if (!warning_matid++)
                Warning("{fl,fr}_matid ignored; may be ok");
        }
#endif
        from_std(&buf.uni_error_mag2); vspl->uni_error_mag2 = buf.uni_error_mag2;
        if (b_nor001) assertx(!vspl->uni_error_mag2);
        from_std(&buf.dir_error_mag2); vspl->dir_error_mag2 = buf.dir_error_mag2;
        from_std(&buf.radius_neg); vspl->radius_neg = buf.radius_neg;
        from_std(&buf.sin2alpha); vspl->sin2alpha = buf.sin2alpha;
        ...
        for_int(c, 3) { from_std(&buf.p[c]); p[c] = buf.p[c]; }
        for_int(c, 3) { from_std(&buf.n[c]); n[c] = buf.n[c]; }
...
//    if dir_error_mag2*mag2(cross(normalized(p-eye), vnormal)) <
//          tz2*mag2(p-eye)
//      [ mag2(cross(unitv1, unitv2)) == (1-square(dot(unitv1, unitv2))) ]
//    if dir_error_mag2*(1-square(dot(normalized(p-eye), vnormal))) < ...
//    if dir_error_mag2*(mag2(p-eye)-square(dot(p-eye, vnormal))) <
//          tz2*mag4(p-eye)
//  unoptimized:
//   const Point& eye = _refp._eye;
//   float rhs1 = _refp._tz2*mag2(p-eye);
//   if (vspl->uni_error_mag2 < rhs1 &&
//       vspl->dir_error_mag2*(mag2(p-eye)-square(dot(p-eye, vs->vnormal)))< rhs1*mag2(p-eye)) return false;
...
void SRMesh::apply_vspl(SRVertex* vs, EListNode*& pn) {
    ...
    if (vta->vmorph && vta->vmorph->coarsening)
        abort_coarsen_morphing(vta->vertex);
    ...
    if (vua->visible && _refine_morph_time) {
        ...
        if (1)         { cp0 = cp[0]; cp1 = cp[1]; cp2 = cp[2]; }
        if (!b_nor001) { cp3 = cp[3]; cp4 = cp[4]; cp5 = cp[5]; }
        if (1)         { lp[0] = cp0; lp[1] = cp1; lp[2] = cp2; }
        if (!b_nor001) { lp[3] = cp3; lp[4] = cp4; lp[5] = cp5; }
        float gp0, gp1, gp2, gp3, gp4, gp5;
        if (1)         { gp0 = ogp[0]; gp1 = ogp[1]; gp2 = ogp[2]; }
        if (!b_nor001) { gp3 = ogp[3]; gp4 = ogp[4]; gp5 = ogp[5]; }
        if (1)         { gp[0] = gp0; gp[1] = gp1; gp[2] = gp2; }
        if (!b_nor001) { gp[3] = gp3; gp[4] = gp4; gp[5] = gp5; }
        if (1)         { ip[0] = (gp0-cp0)*frac; ip[1] = (gp1-cp1)*frac; ip[2] = (gp2-cp2)*frac; }
        if (!b_nor001) { ip[3] = (gp3-cp3)*frac; ip[4] = (gp4-cp4)*frac; ip[5] = (gp5-cp5)*frac; }
    } else {
#if defined(SR_NO_VSGEOM)
...
#endif
        ASSERTX(vra);
        if (vra->vmorph && vra->vmorph->coarsening)
            perhaps_abort_coarsen_morphing(vra->vertex);
    } else {
        fr->aface = &_isolated_aface;    // correction
        fla->fnei[2] = &_isolated_aface; // correction
        --_num_active_faces;                 // anticipating correction
    }
    if (vla->vmorph && vla->vmorph->coarsening)
        perhaps_abort_coarsen_morphing(vla->vertex);
    EListNode* n = pn;
    if (is_splitable(vu)) {
    ...
    _num_active_vertices++; _num_active_faces += 2;
}
...
void SRMesh::apply_ecol(SRVertex* vs, EListNode*& pn) {
    ...
    --_num_active_vertices; --_num_active_faces;
}
...
void SRMesh::extract_gmesh(GMesh& gmesh) const {
    ...
    for_int(fi, _faces.num()) {
        ...
        Face gf = gmesh.create_face_private(fi+1, gvaa); dummy_use(gf);
    ...
    for_int(vi, _vertices.num()) {
        ...
        if (is_active_v(v)) {
            for (SRVertex* vp = v->parent; vp; vp = vp->parent) {
                ...
                for_int(i, 4) {
                    if (vpspl->fn[i]!=&_isolated_face)
                        assertx(is_active_f(vpspl->fn[i]));
                }
            }
            if (is_splitable(v)) {
                int vspli = v->vspli;
...
void SRMesh::set_view_params(const SRViewParams& vp) {
    ...
    if (activate_lr_planes) {
        // Left
        _refp._planes[_refp._nplanes++] = LinearFunc(ok_normalized(cross(frame.p(),
    ...
    if (sr_geomorph_anticipation &&_refp._eyedir.eval(fut_eye)>0.f)
        _refp._eyedir = LinearFunc(frame.v(0), fut_eye);
}
...
void SRMesh::fully_refine() {
    int bu_rmt = _refine_morph_time; set_refine_morph_time(0);
    int bu_cmt = _coarsen_morph_time; set_coarsen_morph_time(0);
...
void SRMesh::verify_optimality() const {
    ...
    for (SRAVertex* vta : EList_outer_range(_active_vertices, SRAVertex, activev)) {
        ...
        if (qcoarsen(vt)) {
            if (!(vta->vmorph && vta->vmorph->coarsening))
                Warning("** should coarsen");
        } else {
            if (vta->vmorph && vta->vmorph->coarsening)
                Warning("** should not be coarsen-morphing");
        }
    }
}
...
void SRMesh::force_vsplit(SRVertex* vsf, EListNode*& n) {
    ...
    while (!stack_split.empty()) {
        ...
        if (has_been_split(vs)) {
            ...
            SRVertex* vp = vs->parent; ASSERTX(vp);
            stack_split.push(vp);
        } else {
            SRVsplit* vspl = &_vsplits[vs->vspli];
            for_int(i, 4) {
                SRFace* fn = vspl->fn[i];
                if (!is_active_f(fn) && fn!=&_isolated_face)
                    stack_split.push(get_vt(get_vspli(fn))->parent);
            }
...
static inline unsigned lsb_mask(unsigned size) {
    int count = 0; while ((size&1)==0) { count++; size >>= 1; }
    return 1u<<count;
}
        ...
        if (is_splitable(vs)) {
            ...
            if (!new_vis) {
                ...
                if (big_error(rvg, cvspl)) {
                    ...
                    EListNode* tn = n->prev(); force_vsplit(vs, tn); n = tn;
                    continue;
                }
...
void SRMesh::update_vmorphs() {
    ...
    for (EListNode* n = ndelim->next(); ; ) {
        ...
        if (vm->time) {
            ...
            if (1)         { ip0 = ip[0]; ip1 = ip[1]; ip2 = ip[2]; }
            if (!b_nor001) { ip3 = ip[3]; ip4 = ip[4]; ip5 = ip[5]; }
            if (1)         { cp[0] = cp0+ip0; cp[1] = cp1+ip1; cp[2] = cp2+ip2; }
            if (!b_nor001) { cp[3] = cp3+ip3; cp[4] = cp4+ip4; cp[5] = cp5+ip5; }
        } else {
            ASSERTX(vm->time==0);
...
bool SRMesh::is_still_morphing() const {
    return _num_vertices_refine_morphing || _num_vertices_coarsen_morphing;
}

bool SRMesh::is_still_adapting() const {
    return _was_modified;
}

int SRMesh::num_vertices_refine_morphing() const {
    return _num_vertices_refine_morphing;
}

int SRMesh::num_vertices_coarsen_morphing() const {
    return _num_vertices_coarsen_morphing;
}
...
void SRMesh::construct_geomorph(SRGeomorphInfo& geoinfo) {
    ...
    for (SRAVertex* va : EList_outer_range(_active_vertices, SRAVertex, activev)) {
        ...
        SRVertex* vv = v;
        while (!m_v_vg.contains(vv))
            vv = assertx(vv->parent);
        if (vv!=v)
            geoinfo._ancestors[0].enter(v, m_v_vg.get(vv));
    }
    m_v_vg.clear();
    ...
    for (SRAVertex* va : EList_outer_range(_active_vertices, SRAVertex, activev)) {
        ...
        while (!m_v_vg.contains(vv))
            vv = assertx(vv->parent);
        if (vv!=v)
            geoinfo._ancestors[1].enter(v, m_v_vg.get(vv));
    }
}
...
void SRMesh::extract_gmesh(GMesh& gmesh, const SRGeomorphInfo& geoinfo) const {
    ...
    for (SRAVertex* va : EList_outer_range(_active_vertices, SRAVertex, activev)) {
        ...
        {
            bool present; const SRVertexGeometry* vg = &geoinfo._ancestors[0].retrieve(v, present);
        ...
        {
            bool present; const SRVertexGeometry* vg = &geoinfo._ancestors[1].retrieve(v, present);
            if (!present) vg = &va->vgeom;
            const Point& p = vg->point;
...
void SRMesh::refine_in_best_dflclw_order() {
    ...
    while (!stcvspl.empty()) {
        ...
        { float fdflclw = float(dflclw); de_dflclw.enter_vector(V(fdflclw)); }
        ...
        --ncand;
        {
            ...
            SRVertex* vl; SRVertex* vr = nullptr;
            SRAFace* fa = vspl->fn[1]->aface;
            vl = fa->vertices[get_vf_j2(vs->avertex, fa)]->vertex;
            fa = vspl->fn[3]->aface;
            if (fa!=&_isolated_aface)
                vr = fa->vertices[get_vf_j1(vs->avertex, fa)]->vertex;
            pncands.push(vl);
        ...
        for (SRVertex* v : pncands) {
            ...
            Scvspl nt; nt.vs = v; nt.iflclw1 = get_iflclw(v)+1;
