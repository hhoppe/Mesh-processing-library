...
void BPointSpatial::enter(Univ id, const Point* pp) {
    Ind ci = point_to_indices(*pp); assertx(indices_inbounds(ci));
    int en = encode(ci);
...
void BPointSpatial::remove(Univ id, const Point* pp) {
    ...
    int ind = -1; for_int(i, ar.num()) { if (ar[i].id==id) { assertx(ind<0); ind = i; } } assertx(ind>=0);
    ar.erase(ind, 1);
...
IPointSpatial::IPointSpatial(int gn, CArrayView<Point> arp) : Spatial(gn), _pp(arp.data()) {
    for_int(i, arp.num()) {
        Ind ci = point_to_indices(arp[i]); assertx(indices_inbounds(ci));
        int en = encode(ci);
