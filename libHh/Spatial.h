...
class Spatial : noncopyable { // abstract class
 ...
 public:
    explicit Spatial(int gn)            : _gn(gn) { assertx(_gn<=k_max_gn); _gni = 1.f/static_cast<float>(_gn); }
 ...
 protected:
    ...
    Ind point_to_indices(const Point& p) const  { Ind ci; for_int(c, 3) { ci[c] = float_to_index(p[c]); } return ci; }
    Point indices_to_point(const Ind& ci) const { Point p; for_int(c, 3) { p[c] = index_to_float(ci[c]); } return p; }
...
void ObjectSpatial<Approx2,Exact2>::add_cell(const Ind& ci, Pqueue<Univ>& pq,
                                             const Point& pcenter, Set<Univ>& set) const {
    int en = encode(ci);
