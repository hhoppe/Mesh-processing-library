...
// True if the input *.pm file was not created with '-minii2 -no_fit_geom'.
// #define SR_NO_VSGEOM
...
// Do not send render faces if provably outside the frustum (see notes in .cpp).
...
struct SrVertexMorph {
  bool coarsening;             // Same as !refining.
  short time;                  // Here, time - 1 is left to achieve the goal (always >= 0).
  SrVertexGeometry vgrefined;  // The right child refined geometry.
  SrVertexGeometry vginc;      // The increment to add during the morph.
...
struct SrVsplit {
  ...
  Vec4<SrFace*> fn;  // Set to &_inactive_face if no face is expected.
...
#if !defined(SR_NOR001)
  float uni_error_mag2;  // Squared magnitude of the uniform residual error.
#else
  static const float uni_error_mag2;  // Always 0.f.
#endif
  float dir_error_mag2;  // Squared magnitude of the directional residual error.
  float radius_neg;      // Maximum radius of influence (negated).
#if !defined(SR_NOR001)
...
struct SrRefineParams {
  ...
  float _tz2;                  // The screen-space tolerance, zoomed and squared.
  LinearFunc _eyedir;          // A linear func along the view direction.
};
...
class SrViewParams {
 public:
  ...
  // Here, zoomyx[1] is tan(angle), where angle is between frame.v(0) and the left viewport edge.
  // Here, zoomyx[0] is tan(angle), where angle is between frame.v(0) and the top viewport edge.
  void set_zooms(const Vec2<float>& zoomyx);  // No default.
  // Enable the left and right view frustum planes.
  void activate_lr_planes(bool flag);  // Default true.
  // Enable the bottom view frustum plane.
  void activate_bottom_plane(bool flag);  // Default true.
  // Enable the top view frustum plane.
...
class SrMesh {
 public:
  ...
  void fully_refine();
  void fully_coarsen();
  void set_refine_morph_time(int refine_morph_time);    // Here, 0 = disable.
  void set_coarsen_morph_time(int coarsen_morph_time);  // Here, 0 = disable.
 ...
 private:
  ...
  static SrFace _isolated_face;  // Used for fn[*] when no neighbor is expected.
  // Properties: matid == illegal, fnei[*] == &_isolated_aface.
  static SrAFace _isolated_aface;  // Used for fr->activef when !creates_2faces.
  ...
  static constexpr unsigned k_Face_visited_mask = 1 << 30;  // The high bit of matid.
  ...
  [[nodiscard]] bool ecol_legal(const SrVertex* vt) const;  // Here, vt is the left child of its parent!
