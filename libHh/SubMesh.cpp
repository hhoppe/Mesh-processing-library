...
bool Combvh::is_combination() const { return abs(c.sum() + h[3] - 1.f) < 1e-6f; }

Point Combvh::evaluate(const GMesh& mesh) const {
  Homogeneous th(h);
  for_combination(c, [&](Vertex vv, float val) { th += val * Homogeneous(mesh.point(vv)); });
  return to_Point(th);
}
...
void SubMesh::convolve_self(const Mvcvh& mconv) { _cmvcvh.compose(mconv); }

const Combvh& SubMesh::combination(Vertex v) const { return _cmvcvh.get(v); }
...
void SubMesh::show_cmvcvh() const { show_mvcvh(_cmvcvh); }

// *** helper

bool SubMesh::sharp(Edge e) const { return _m.is_boundary(e) || _m.flags(e).flag(GMesh::eflag_sharp); }

int SubMesh::nume(Vertex v) const { return vinfo(v).nume; }

int SubMesh::num_sharp_edges(Vertex v) const { return vinfo(v).numsharpe; }
