  ...
  if (1) {
    ...
    if (RegOpenKeyExA(HKEY_LOCAL_MACHINE, R"(HARDWARE\DESCRIPTION\System\CentralProcessor\0)", 0, KEY_QUERY_VALUE,
    ...
    } else {
      {
        ...
        if (rev == "" && !RegQueryValueExA(hkey, "~MHz", nullptr, &tdword, reinterpret_cast<uchar*>(&val), &len)) {
          assertx(len == 4 && val > 0);
          rev = sform("%dMHz", val);
        }
      }
      {
        ...
        if (rev == "" && !RegQueryValueExA(hkey, "ProcessorNameString", nullptr, nullptr,
          ...
          rev = p;
          // replace spaces by '_'
          for_int(i, narrow_cast<int>(rev.size())) {
            if (rev[i] == ' ') rev[i] = '_';
          }
        }
      }
...
class Timers {
 ...
 private:
      ...
      for (const auto& timer_info : _vec_timer_info) {
        ...
        if (timer_info.sum_process_time) {
          ...
          if (meaningful) {
            ...
            if (1 && timer_info.sum_process_time > stat_sum) stat_sum = timer_info.sum_process_time;
            if (0 && timer_info.sum_process_time / ncores > stat_sum) stat_sum = timer_info.sum_process_time / ncores;
          }
        }
        int64_t n = stat.num();
  ...
  struct TimerInfo {
    ...
    TimerInfo(TimerInfo&& t) noexcept
