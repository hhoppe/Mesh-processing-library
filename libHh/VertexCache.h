...
class FifoVertexCache : public VertexCache {
 public:
    ...
    bool access_hits(int vi) override {
        ...
        if (0) {
            if (_vinqueue[vi]>=0)
                return true;        // if there, do nothing
            if (_iprev==0) {
    ...
    bool contains(int vi) const override {
        ASSERTX(vi>=1 && vi<_vinqueue.num()); return _vinqueue[vi]>=0;
    }
    ...
    unique_ptr<VertexCache::Iter> make_iterator() const override {
        return make_unique<Iter>(*this);
    }
 private:
    class Iter : public VertexCache::Iter {
     public:
        Iter(const FifoVertexCache& vcache)
            : _qv(vcache._queuev), _qi(vcache._iprev), _numi(vcache._queuev.num()) { }
        int next() override {
            if (!_numi) return 0;
            --_numi;
            int vi = _qv[_qi];
...
class LruVertexCache : public VertexCache {
 public:
    ...
    bool contains(int vi) const override {
        ASSERTX(vi>=1 && vi<_vinlist.num()); return !!_vinlist[vi];
    }
    int location(int vi) const override {
        ASSERTX(vi>=1 && vi<_vinlist.num());
        if (!_vinlist[vi])
 ...
 private:
    class Iter : public VertexCache::Iter {
     public:
        Iter(const LruVertexCache& vcache)
            : _delim(vcache._list.delim()), _n(vcache._list.delim()->next()) { }
        int next() override {
...
inline std::ostream& operator<<(std::ostream& os, const VertexCache& vc) {
    auto up_vci = vc.make_iterator(); VertexCache::Iter& vci = *up_vci;
    os << "{";
    for (;;) {
        int vi = vci.next(); if (!vi) break;
        os << sform(" %d", vi);
    }
    return os << "}";
}
