 ...
 public:
  ...
  void write(CMatrixView<Pixel> frame) override {
    ...
    com_ptr<IWICBitmap> bitmap;
    {
      ...
      AS(wic_factory->CreateBitmapFromMemory(sdims[1], sdims[0], bitmap_pixel_format, stride, buffer_size, buf,
                                             &bitmap));
    }
    // requires Windows 8
    com_ptr<IMFMediaBuffer> pBuffer;
    AS(MFCreateWICBitmapBuffer(__uuidof(IWICBitmap), bitmap, &pBuffer));
#else
 ...
 public:
  ...
  void write(CMatrixView<Pixel> frame) override {
    ...
    if (!_wvideo._use_nv12) {
      if (!write_binary_raw((*_pfi)(), frame.array_view())) throw std::runtime_error("FF_WVideo write frame failed");
    } else {
