...
void test(MatrixView<float> mi) {
    ...
    subtract_mean(mi);
    // SHOW(mi);
    Matrix<float> mo(mi.xsize(), mi.xsize()); Array<float> eimag(mi.xsize());
...
void read_data(Matrix<float>& mi) {
    ...
    mi.init(m, n);
    for_int(j, n) for_int(i, m) {
        assertx(fi() >> mi[i][j]);
    }
    {
        float dummy; fi() >> dummy; assertx(!fi());
    }
}

void test_inc() {
    Matrix<float> mi; read_data(mi);
    int n = mi.xsize();
    const int ne = 8;
    ...
    { HH_TIMER(_pc2); incr_principal_components(mi, mo2, eimag2, 2); }
    for_int(i, ne) {
...
void test_em() {
    ...
    {
        double var = 0.f; for_int(y, mi.ysize()) { var += mag2(mi[y]); }
        SHOW(var/mi.ysize());
    }
    for_int(i, ne) {
        float d = float(dot(mo1[i], mo2[i]));
