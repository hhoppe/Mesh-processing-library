...
void test2D() {
    ...
    auto func_polylinetoa3d = [](const Array<Vec2<float>>& poly, A3dElem& el) {
        ...
        for_int(i, poly.num()) {
            el.push(A3dVertex(Point(0.f, poly[i][0], poly[i][1]), Vector(0.f, 0.f, 0.f),
                              A3dVertexColor(Pixel::red())));
        }
    };
    int gn = 20;
    WFile fcontour("tContour.2D");
    WSA3dStream wcontour(fcontour());
    WFile fborder("tContour.2Dborder");
    WSA3dStream wborder(fborder());
    A3dElem el;
    auto func_contour = [&](const Array<Vec2<float>>& poly) {
        func_polylinetoa3d(poly, el); wcontour.write(el);
    };
    auto func_border = [&](const Array<Vec2<float>>& poly) {
        func_polylinetoa3d(poly, el); wborder.write(el);
    };
    Contour2D<feval2D, decltype(func_contour), decltype(func_border)>
        contour(gn, feval2D(), func_contour, func_border);
...
void test3D() {
    int gn = 10;
    WFile fcontour("tContour.3D");
    WSA3dStream wcontour(fcontour());
    WFile fborder("tContour.3Dborder");
    WSA3dStream wborder(fborder());
    auto func_polygontoa3d = [](const Array<Vec3<float>>& poly, A3dElem& el) {
        el.init(A3dElem::EType::polygon);
        for_int(i, poly.num()) {
            el.push(A3dVertex(poly[i], Vector(0.f, 0.f, 0.f), A3dVertexColor(Pixel::red())));
        }
    };
    A3dElem el;
    auto func_contour = [&](const Array<Vec3<float>>& poly) {
        func_polygontoa3d(poly, el); wcontour.write(el);
    };
    auto func_border = [&](const Array<Vec3<float>>& poly) {
        func_polygontoa3d(poly, el); wborder.write(el);
    };
    Contour3D<feval3D, decltype(func_contour), decltype(func_border)>
        contour(gn, func_contour, feval3D(), func_border);
