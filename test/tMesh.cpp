...
int main() {
    ...
    //
    Face f0, f1;
    f0 = mesh.create_face(V(v1, v2, v3)); dummy_use(f0);
    show_mesh(mesh);
    assertx(!mesh.legal_create_face(V(v1, v2, v4)));
    f1 = mesh.create_face(v2, v1, v4); dummy_use(f1);
    show_mesh(mesh);
    mesh.ok();
    //       2
    //   3   |   4
    ...
    }
    {
        ...
        { Face f = mesh.face1(e); assertx(f==f531 || f==mesh.face(v1, v4/*, v5*/)); }
        { Face f = mesh.face2(e); assertx(f==f531 || f==mesh.face(v1, v4/*, v5*/)); }
        { Vertex v = mesh.side_vertex1(e); assertx(v==v3 || v==v4); }
        { Vertex v = mesh.side_vertex2(e); assertx(v==v3 || v==v4); }
    ...
    {                           // all four faces should have exactly two neighbors
        for (Face f : mesh.faces()) {
            int count = 0; for (Face ff : mesh.faces(f)) { dummy_use(ff); count++; } assertx(count==2);
        }
    }
    ...
    {
        Array<Vertex> va; for_int(i, 3) { va.push(mesh.create_vertex()); }
        Face f = mesh.create_face(va);
        for (Face ff : mesh.faces(f)) { dummy_use(ff); if (1) assertnever(""); }
    }
    SHOW("all ok");
}
