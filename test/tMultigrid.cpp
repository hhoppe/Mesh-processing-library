...
void setup_rhs(CGridView<D,T> grid_orig, Multigrid<D, T, Periodic, Metric>& multigrid) {
    ...
    Periodic periodic;
    parallel_for_coords(dims, [&](const Vec<int,D>& u) {
        T vrhs; my_zero(vrhs);
        for_int(c, D) {
            if (u[c]>0)           { vrhs += grid_orig[u.with(c, u[c]-1)]   -grid_orig[u]; }
            else if (periodic(c)) { vrhs += grid_orig[u.with(c, dims[c]-1)]-grid_orig[u]; }
...
template<typename T> void setup_rhs2(CGridView<2,T> grid_orig, GridView<2,T> grid_rhs,
                                     float gradient_sharpening = 1.f, float screening_weight = 0.f) {
...
template<int D> void test_random(Args& args) {
    Vec<int,D> dims; for_int(c, D) { dims[c] = args.get_int(); }
    // using FType = float;
    using FType = double;
...
void test(GridView<D,T> grid_orig, Periodic = Periodic{}) {
    ...
    auto expected_rms_err = max(dims)*10.f*(std::is_same<decltype(max_e(T())), float>::value ? 1e-7 : 1e-16);
    if (0) SHOW(expected_rms_err, result_rms_err);
    if (result_rms_err>=expected_rms_err) { SHOW(expected_rms_err, result_rms_err); assertnever(""); }
}
...
int main(int argc, const char** argv) {
    ...
    if (0) {                    // debug the iterators with normal and interior functions
        ...
        SHOW("beg"); for_2DL_interior(0, 4, 0, 4, func_norm, func_interior);
        SHOW("beg"); for_2DL_interior(0, 2, 0, 4, func_norm, func_interior);
        SHOW("beg"); for_2DL_interior(0, 1, 0, 4, func_norm, func_interior);
        SHOW("beg"); for_2DL_interior(0, 0, 0, 4, func_norm, func_interior);
        SHOW("beg"); for_2DL_interior(0, 5, 3, 3, func_norm, func_interior);
        SHOW("beg"); for_2DL_interior(0, 5, 3, 4, func_norm, func_interior);
        SHOW("beg"); for_2DL_interior(0, 5, 3, 5, func_norm, func_interior);
        SHOW("beg"); for_2DL_interior(0, 5, 3, 6, func_norm, func_interior);
        return 0;
    }
    if (0) {                    // debug the iterators with normal and interior functions
        ...
        SHOW("beg"); for_coordsL_interior(V(5, 5), V(0, 0), V(5, 5), func_norm, func_interior);
        SHOW("beg"); for_coordsL_interior(V(5, 5), V(2, 0), V(4, 5), func_norm, func_interior);
        SHOW("beg"); for_coordsL_interior(V(5, 5), V(0, 0), V(2, 4), func_norm, func_interior);
        return 0;
    }
    if (1) {                    // reconstruct noise grid from its Laplacian
        ...
        if (0) { }
        else if (args.num()==1) { test_random<1>(args); return 0; }
        else if (args.num()==2) { test_random<2>(args); return 0; }
        else if (args.num()==3) { test_random<3>(args); return 0; }
        else if (args.num()>0) { assertnever(""); }
    }
    if (0) {                    // gradient sharpening
        ...
        Grid<2,Vector4> grid_orig; read_image(image, grid_orig);
        ...
        if (1) {
            ...
            as_image(Vector4(.5f)+standardize_rms(Grid<2,Vector4>(grid_rhs))*Vector4(.15f)).
                write_file("image_rhs.bmp");
        }
    ...
    if (0) {                    // reconstruct image from its Laplacian
        ...
        CGridView<2,Vector4> grid_result = multigrid.result();
        as_image(grid_result).write_file("image_result.bmp");
        if (1) as_image(Vector4(.5f)+standardize_rms(grid_result-grid_orig)*Vector4(.1f)).
                   write_file("image_err.bmp");
    }
    if (1) {                    // stitch two images together
        string imagename0, imagename1;
        imagename0 = "multigrid/rampart237.png";  imagename1 = "multigrid/prismatic237.png";
        imagename0 = "multigrid/rampart2048.png"; imagename1 = "multigrid/prismatic2048.png";
        Vec3<Grid<2,Vector4>> grids;
        Image image0; image0.read_file(imagename0); read_image(image0, grids[0]);
        Image image1; image1.read_file(imagename1); read_image(image1, grids[1]);
        ...
        for_int(c, 3) {
            Multigrid<2,float> multigrid(dims); double mean_orig; {
