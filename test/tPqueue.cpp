...
int main() {
    ...
    {
        Pqueue<int> pq;
        Pqueue<int> pq2 = pq; dummy_use(pq2);
    }
#endif
#if 0                           // test noncopyable
    {
        Pqueue<int> pq;
        Pqueue<int> pq2 = std::move(pq); dummy_use(pq2);
    }
#endif
    {
        Pqueue<int> pq;
    ...
    }
    {
        for_int(k, 500) {
            ...
            Array<float> arval1; for_int(i, n) { arval1.push(2.f+sin(i*11.f+k*1.2345f)); }
            Array<float> arval2; for_int(i, n) { arval2.push(2.f+sin(i*13.f+k*2.7419f)); }
            Array<float> arval3; for_int(i, n) { arval3.push(2.f+sin(i*13.f+k*3.1415f)); }
    ...
    }
    {
        ...
        std::default_random_engine dre;
        for_int(k, ntests) {
            Array<int> ar1; for_int(i, n) { ar1.push(i); } std::shuffle(ar1.begin(), ar1.end(), dre);
            Array<float> ar2; for_int(i, n) { ar2.push(float(i)); } std::shuffle(ar2.begin(), ar2.end(), dre);
            Array<float> ar3; for_int(i, n) { ar3.push(float(i)); } std::shuffle(ar3.begin(), ar3.end(), dre);
            HPqueue<int> hpq;
            for_int(i, n) {
                int j = ar1[i];
                hpq.enter(j, ar2[j]);
